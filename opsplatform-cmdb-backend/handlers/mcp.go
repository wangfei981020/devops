...
var mcpTools = []mcpTool{
	...
	{"gke_available_versions", "某集群所在区域可选的升级目标版本(GKE getServerConfig 权威清单,按官方降序)。给 gke_upgrade_plan 填 target_version 前先查这个,别猜版本号。⚠️ versions 为空是「还没采集」不是「没有可用版本」,看 note 字段", "/api/gke/available-versions", []mcpParam{{"cluster_id", "integer", "集群ID", true}, {"kind", "string", "master=控制面(默认) node=节点池", false}}, false},
	...
	{"pod_logs", "读取 Pod 日志(尾部N行)——诊断服务问题用。CrashLoopBackOff/重启次数高的 Pod 必须传 previous=1,否则拿到的是刚起来的空日志", "/api/k8s/pod-logs", []mcpParam{{"cluster_id", "integer", "集群ID", true}, {"namespace", "string", "命名空间", true}, {"pod", "string", "Pod名", true}, {"container", "string", "容器", false}, {"tail", "integer", "行数默认200", false}, {"previous", "string", "1=取上一个已崩溃容器实例的日志(CrashLoop 必用)", false}}, true},
	...
	{"list_cdn_rules", "CDN 规则台账(Page Rules + Rulesets):缓存/强制HTTPS/WAF自定义规则。排查'为什么这个路径没走缓存'用它", "/api/cdn/rules", []mcpParam{{"zone", "string", "根域名", false}, {"source", "string", "pagerule 或 ruleset", false}}, false},
	{"cdn_rule_analysis", "CDN 规则优化分析:数量逼近套餐上限/禁用规则占额度/同一匹配式重复配置(只有优先级最高的生效)", "/api/cdn/rule-analysis", []mcpParam{{"zone", "string", "根域名", false}}, false},
	{"list_cdn_certificates", "CDN 边缘证书(Cloudflare Universal SSL 等)+到期天数。与 list_certificates(我方源站证书)是两套,到期时间互相独立", "/api/cdn/certificates", []mcpParam{{"zone", "string", "根域名", false}}, false},
	{"cloud_iam_audit", "GCP 项目权限审计:谁对项目有什么角色,标出过宽权限(owner/editor/可自行提权)与 allUsers 公开授权。only=issues 只看有风险的", "/api/cloud-iam", []mcpParam{{"project", "string", "项目ID", false}, {"only", "string", "issues=只看有风险的", false}}, false},
	{"list_cloud_dns", "GCP Cloud DNS 托管区与解析记录——与 list_cdn_dns(Cloudflare) 是两套解析,排查'改了没生效'时两边都要看", "/api/cloud-dns", []mcpParam{{"project", "string", "项目ID", false}, {"q", "string", "关键词", false}}, false},
	{"dns_consistency", "GCP Cloud DNS vs Cloudflare 解析一致性:同一域名两边都配且目标不一致时列出——只有 NS 指向的那边才生效,另一边改了没用", "/api/dns-consistency", nil, false},
	{"security_audit", "容器安全审计:特权容器/hostPath/hostNetwork/capabilities/以root运行。默认隐藏平台组件(CNI/CSI等特权是设计使然),include_platform=1 可全列", "/api/k8s/security-audit", []mcpParam{{"cluster_id", "integer", "集群ID", true}, {"namespace", "string", "命名空间", false}, {"include_platform", "string", "1=含平台组件", false}}, false},
