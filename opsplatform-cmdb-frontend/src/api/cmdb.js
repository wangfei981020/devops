...
// K8s 集群纳管（只读多集群）
export const listK8sClusters = () => http.get('/k8s/clusters').then((r) => r.data)
export const createK8sCluster = (b) => http.post('/k8s/clusters', b).then((r) => r.data)
export const updateK8sCluster = (id, b) => http.put('/k8s/clusters/' + id, b).then((r) => r.data)
...
export const listK8sNamespaces = (params) => http.get('/k8s/namespaces', { params }).then((r) => r.data)
export const listK8sWorkloads = (params) => http.get('/k8s/workloads', { params }).then((r) => r.data)
export const listK8sPods = (params) => http.get('/k8s/pods', { params }).then((r) => r.data)
export const listK8sServices = (params) => http.get('/k8s/services', { params }).then((r) => r.data)
export const listK8sIngresses = (params) => http.get('/k8s/ingresses', { params }).then((r) => r.data)
export const listK8sGateways = (params) => http.get('/k8s/gateways', { params }).then((r) => r.data)
export const listK8sHTTPRoutes = (params) => http.get('/k8s/httproutes', { params }).then((r) => r.data)
export const listK8sVirtualServices = (params) => http.get('/k8s/virtualservices', { params }).then((r) => r.data)
export const k8sPodUsage = (params) => http.get('/k8s/pod-usage', { params }).then((r) => r.data)
export const k8sNodeUsage = (params) => http.get('/k8s/node-usage', { params }).then((r) => r.data)
export const k8sPvcUsage = (params) => http.get('/k8s/pvc-usage', { params }).then((r) => r.data)
export const k8sNodeCapacity = (params) => http.get('/k8s/node-capacity', { params }).then((r) => r.data)
...
// 全链路：正向(域名→…→节点) / 反向(节点→受影响域名)
export const k8sTopology = (domain) => http.get('/k8s/topology', { params: { domain } }).then((r) => r.data)
export const k8sImpact = (cluster_id, node) => http.get('/k8s/impact', { params: { cluster_id, node } }).then((r) => r.data)
export const listK8sChanges = (params) => http.get('/k8s/changes', { params }).then((r) => r.data)
// 合并 k8sinsight：实时日志/事件/规则诊断
export const k8sPodLogs = (params) => http.get('/k8s/pod-logs', { params }).then((r) => r.data)
export const k8sPodEvents = (params) => http.get('/k8s/pod-events', { params }).then((r) => r.data)
export const k8sDiagnose = (params) => http.get('/k8s/diagnose', { params }).then((r) => r.data)
export const k8sEvents = (params) => http.get('/k8s/events', { params }).then((r) => r.data)
// 数据源接入
export const listObsEndpoints = () => http.get('/obs-endpoints').then((r) => r.data)
export const createObsEndpoint = (b) => http.post('/obs-endpoints', b).then((r) => r.data)
export const updateObsEndpoint = (id, b) => http.put('/obs-endpoints/' + id, b).then((r) => r.data)
export const deleteObsEndpoint = (id) => http.delete('/obs-endpoints/' + id).then((r) => r.data)
export const testObsEndpoint = (id) => http.post(`/obs-endpoints/${id}/test`).then((r) => r.data)
// CDN 接入（Cloudflare 只读）。注意与「注册商」里的 cloudflare 不是一回事：
// 注册商那份用于证书 DNS-01 验证，需要写 TXT 记录的权限；这里只读，用来采 Zone/DNS/配置。
export const listCdnAccounts = () => http.get('/cdn/accounts').then((r) => r.data)
export const saveCdnAccount = (b) => http.post('/cdn/accounts', b).then((r) => r.data)
...
// AI 接入 (MCP)
export const mcpInfo = () => http.get('/mcp/info').then((r) => r.data)
export const mcpRegenerate = () => http.post('/mcp/regenerate').then((r) => r.data)
export const listK8sNsProjects = (cluster_id) => http.get('/k8s/ns-projects', { params: { cluster_id } }).then((r) => r.data)
export const setK8sNsProject = (b) => http.post('/k8s/ns-projects', b).then((r) => r.data)
// 云成本
export const costOverview = (params) => http.get('/k8s/cost/overview', { params }).then((r) => r.data)
export const costDetail = (params) => http.get('/k8s/cost/detail', { params }).then((r) => r.data)
export const costNodes = () => http.get('/k8s/cost/nodes').then((r) => r.data)
export const setNodeCostOverride = (b) => http.post('/k8s/cost/node-override', b).then((r) => r.data)
export const costSnapshot = (month) => http.post('/k8s/cost/snapshot', null, { params: { month } }).then((r) => r.data)
export const costMonths = () => http.get('/k8s/cost/months').then((r) => r.data)
export const costReport = (params) => http.get('/k8s/cost/report', { params }).then((r) => r.data)
export const costAttribution = (month) => http.get('/k8s/cost/attribution', { params: { month } }).then((r) => r.data)
