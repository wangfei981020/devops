...
import { listProjects, createProject, updateProject, deleteProject, listEnvironments, createEnvironment, updateEnvironment, deleteEnvironment } from '../api/cmdb'
import { useAppStore } from '../stores/app'

const app = useAppStore()
const tagTypes = [{ v: 'danger', l: '红' }, { v: 'warning', l: '橙' }, { v: 'primary', l: '蓝' }, { v: 'success', l: '绿' }, { v: 'info', l: '灰' }]
const projects = ref([]), envs = ref([])
const pDlg = ref(false), pEdit = ref(false), pForm = ref({})
const eDlg = ref(false), eEdit = ref(false), eForm = ref({})

async function load() { projects.value = await listProjects(); envs.value = await listEnvironments() }
