...
func (g *GCP) resolveLBBackends(ctx context.Context, svc *compute.Service, projectID string, lbs []LoadBalancer, lim *limiter) {
	...
	try := func(label string, fn func() error) {
		if err := g.retry(ctx, lim, fn); err != nil {
			logx.Line("lb-backend", fmt.Sprintf("WARN 项目 %s 拉取 %s 失败: %v", projectID, label, err))
			degraded = true
		}
	}
	...
	for i := range lbs {
		...
		if len(backends) == 0 && lb.Target != "" {
			logx.Line("lb-backend", fmt.Sprintf("WARN LB %s (project=%s, target=%s) 未追溯到后端实例：命中 targetPool=%d backendService=%d（可能是无实例后端/服务型NEG，或状态不支持）",
