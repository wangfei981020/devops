...
func (g *GCP) ListIAM(ctx context.Context, projectID string) ([]IAMBinding, error) {
	...
	if err != nil {
		logx.Line("gcp-iam-dns", fmt.Sprintf("ERROR project=%s 创建 cloudresourcemanager 客户端失败: %v", projectID, err))
		return nil, err
	}
	...
	if err != nil {
		...
		logx.Line("gcp-iam-dns", fmt.Sprintf("ERROR project=%s 取 IAM 策略失败: %v（若为 403，服务账号需要 roles/iam.securityReviewer 或 roles/viewer）", projectID, err))
		return nil, err
	}
	if policy == nil {
		logx.Line("gcp-iam-dns", fmt.Sprintf("WARN project=%s 返回空策略——这不正常，任何项目至少有一条绑定，请确认凭据对应的项目是否正确", projectID))
		return nil, nil
	}
	...
	logx.Line("gcp-iam-dns", fmt.Sprintf("project=%s 取到 %d 条绑定（%d 个角色）", projectID, len(out), len(policy.Bindings)))
	return out, nil
}
...
func (g *GCP) ListDNS(ctx context.Context, projectID string) ([]DNSZone, error) {
	...
	if err != nil {
		logx.Line("gcp-iam-dns", fmt.Sprintf("ERROR project=%s 创建 dns 客户端失败: %v", projectID, err))
		return nil, err
	}
	lim := limiterFor(projectID)
	...
	if err != nil {
		logx.Line("gcp-iam-dns", fmt.Sprintf("ERROR project=%s 列托管区失败: %v（若为 403，服务账号需要 roles/dns.reader；若为 SERVICE_DISABLED，该项目未启用 Cloud DNS API）", projectID, err))
		return nil, err
	}
	if len(zones) == 0 {
		// 空是合法的（该项目可能确实不用 Cloud DNS），但要说出来，
		// 免得「查不到解析」被当成「解析不存在」。
		logx.Line("gcp-iam-dns", fmt.Sprintf("project=%s 没有托管区——该项目可能不使用 Cloud DNS（解析可能在 Cloudflare 或别处）", projectID))
		return nil, nil
	}
	...
	for _, z := range zones {
		...
		if e != nil {
			...
			logx.Line("gcp-iam-dns", fmt.Sprintf("WARN project=%s zone=%s 取解析记录失败: %v（该区记录本次缺失）", projectID, z.Name, e))
			out = append(out, dz)
			continue
		}
		for _, rs := range sets {
	...
	logx.Line("gcp-iam-dns", fmt.Sprintf("project=%s 取到 %d 个托管区", projectID, len(out)))
	return out, nil
}
