...
func TestBackendAlive(t *testing.T) {
	alive := map[string]bool{
		"app-user/user-client-backend-svc": true,
		"devops/nacos":                     false, // Service 还在，但没有存活 Pod
	}
	cases := []struct {
		name, backends, ns, want string
	}{
		{"全限定域名解析到存活服务", "user-client-backend-svc.app-user.svc.cluster.local", "app-user", "yes"},
		{"服务存在但无后端", "nacos.devops.svc.cluster.local", "devops", "no"},
		{"集群内不存在的服务", "external-api.example.com", "app-user", "unknown"},
		{"空后端", "", "app-user", "unknown"},
		{"多后端只要有一个活着就算活", "nacos.devops.svc.cluster.local,user-client-backend-svc.app-user.svc.cluster.local", "devops", "yes"},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
