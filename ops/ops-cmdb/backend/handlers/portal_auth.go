...
func (h *AuthHandler) PortalAuth(c *gin.Context) {
	...
	if !ok {
		...
		httpx.FailKey(c, httpx.CodeUnauthorized, "error.credentialInvalidOrExpired", nil, nil)
		return
	}
	...
	if role != "admin" {
		if !okPerm {
			...
			httpx.FailKey(c, httpx.CodeUpstreamError, "error.permServiceUnavailable", nil, nil)
			return
		}
		if !perms["menu:cmdb"] {
			WriteAuditAs(h.DB, h.ctxWithUser(c, username), "auth.portal.denied", "缺少 menu:cmdb")
			httpx.FailKey(c, httpx.CodeForbidden, "error.noCmdbAccess", nil, nil)
			return
		}
		...
		if !allowed {
			...
			httpx.FailKey(c, httpx.CodeForbidden, "error.roleHasNoCmdbAccess", nil, nil)
			return
		}
	} else if !okPerm {
	...
	if err != nil {
		...
		httpx.FailKey(c, httpx.CodeInternal, "error.createUserFailed", err, nil)
		return
	}
...
func (h *AuthHandler) RefreshPermissions(c *gin.Context) {
	...
	if !perms["menu:cmdb"] {
		...
		httpx.FailKey(c, httpx.CodeForbidden, "error.cmdbAccessRevoked", nil, nil)
		return
	}
