...
func (h *CDNHandler) RegisterRules(r *gin.RouterGroup) {
	...
	// ⚠️ 下面两个实时调 Cloudflare，不读库快照（其余都读库），见各自文件顶部说明
	r.GET("/cdn/token-check", h.ProbeToken)            // zone?, account_id?
	r.GET("/cdn/security-events", h.CDNSecurityEvents) // zone, host?, since?, until?, minutes?, limit?
	r.GET("/cdn/traffic", h.CDNTraffic)                // zone, host?, path?, client_ip?, since?, until?, min_origin_ms?
}
