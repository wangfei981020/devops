...
func fwHighRisk(direction, action, protocols, sourceRanges string) bool {
	risky, _ := fwRisk(direction, action, protocols, sourceRanges)
	return risky
}

// fwRisk 同 fwHighRisk，但把**为什么高危**一起返回。
//
//	# 为什么理由必须由后端给
//
//	界面上一个红色的「高危」标签，人的下一个问题一定是"高危在哪"。
//	答案要么由判定方给出，要么由看的人自己重新推一遍——而重新推的时候
//	他手上只有列表上那几列，推不出"6379 是 Redis 默认无认证"这种知识。
//	判据和理由必须同源：分开写必然分叉，且分叉时理由通常还停在旧口径上，
//	于是理由和标红互相矛盾（本项目在暴露面判定上已经吃过这个亏）。
//
//	返回的理由要能直接读懂：说清「什么方向 + 什么源 + 开了什么」三件事，
//	而不是只说"命中规则 3"。
func fwRisk(direction, action, protocols, sourceRanges string) (bool, string) {
	...
	if p == "" || strings.Contains(p, "all") {
		// 全放行：这条比"开了某个敏感端口"更严重，理由要说满
		return true, "入站放行，来源是任意地址（0.0.0.0/0），且**未限制端口** —— 等于把这批机器的所有端口对公网打开"
	}
	if hit := fwSensitiveHits(p); len(hit) > 0 {
		return true, fmt.Sprintf("入站放行，来源是任意地址（0.0.0.0/0），开放了敏感端口：%s", strings.Join(hit, "、"))
	}
	return false, ""
}

// fwSensitiveHits 命中的敏感端口，形如 ["6379 Redis", "3306 MySQL"]。
//
//	按端口号排序输出：map 遍历顺序是随机的，不排序的话同一条规则
//	每次刷新理由里的端口顺序都在变，看起来像数据在跳。
func fwSensitiveHits(protocols string) []string {
	found := map[int]bool{}
	for _, seg := range strings.Split(protocols, ";") {
		...
		if !hasPorts || strings.TrimSpace(ports) == "" {
			// 裸 tcp/udp = 未限端口，等于全部敏感端口都开着。
			// 这里不逐个列举（列 24 个没人看），交回上层按"未限端口"表述
			return []string{"未限端口（等同全部开放）"}
		}
	...
	for _, p := range ports {
		out = append(out, fmt.Sprintf("%d %s", p, sensitivePortNames[p]))
	}
	return out
}
...
func (h *NetworkHandler) ListFirewalls(c *gin.Context) {
	...
	for rows.Next() {
		...
		if rows.Scan(&provider, &aid, &project, &name, &network, &direction, &priority, &action, &protocols, &src, &tags, &disabled, &hr) == nil {
			...
			live, reason := fwRisk(direction, action, protocols, src)
			// ⚠️ source_ranges / target_tags 在库里是**逗号分隔字符串**（入库时 strings.Join 过），
			// 但接口必须给**数组** —— 逗号分隔是存储细节，不该泄漏到 API。
			//
			// 原来直接把那个字符串塞进 JSON，而前端类型声明的是 string[]：
			// `(r.source_ranges ?? []).join(', ')` 里 `?? []` 只挡 null/undefined，
			// 字符串有值时原样穿过去 → `.join is not a function` → **整页崩溃**（OPSCMDB-012）。
			// 而且这个崩溃被另一个 bug 掩护了很久：数据为空时 .join 根本跑不到，
			// 页面看起来只是"没有数据"。
			item := gin.H{"provider": provider, "project": pn[itoa(aid)+"/"+project], "name": name, "network": network,
				"direction": direction, "priority": priority, "action": action, "protocols": protocols,
				"source_ranges": splitCSV(src), "target_tags": splitCSV(tags), "disabled": disabled == 1, "high_risk": live}
			// 高危时把**为什么**一起给出去。一个红标签不带理由，看的人只能自己猜
			if reason != "" {
...
func (h *NetworkHandler) lbVIPToK8sService() map[string]string {
	out := map[string]string{}
	rows, err := h.DB.Query(`SELECT s.external_ip, s.namespace, s.name, COALESCE(c.name, ''), COALESCE(c.display_name, c.name, '')
	...
	defer rows.Close()
	for rows.Next() {
		var ip, ns, name, clName, clDisp string
		if rows.Scan(&ip, &ns, &name, &clName, &clDisp) != nil {
			continue
		}
		// 拼给人看的单串，用全站统一口径（技术名必须可见）
