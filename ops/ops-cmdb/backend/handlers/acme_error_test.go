...
func TestClassifyAcmeError_生产真实报文(t *testing.T) {
	cases := []struct {
		...
		{
			name: "域名没有公网解析", cn: "*.staging.example.com", challenge: "dns-01",
			raw: "obtain: error: one or more domains had a problem: [*.staging.example.com] " +
				"invalid authorization: acme: error: 400 :: urn:ietf:params:acme:error:dns :: " +
				"DNS problem: NXDOMAIN looking up TXT for _acme-challenge.test-uat.com",
			wantCode: "dns_nxdomain", wantRetryable: false,
		},
		{
			name: "内网域名找公网 CA 签", cn: "*.inner.k8s-app-uat.com", challenge: "dns-01",
			raw: "obtain: error: one or more domains had a problem: [*.inner.k8s-app-uat.com] " +
		...
		{
			name: "手动模式没人加 TXT 记录", cn: "*.k8s-app-uat.com", challenge: "manual-dns",
			raw: "obtain: error: one or more domains had a problem: [*.k8s-app-uat.com] " +
				"[*.k8s-app-uat.com] acme: error presenting token: 等待手动添加 DNS TXT 记录超时（20 分钟）",
...
func TestIsInternalName(t *testing.T) {
	internal := []string{"*.inner.k8s-app-uat.com", "svc.internal.corp", "db.local", "app.intra.net", "x.lan"}
	...
	public := []string{"*.staging.example.com", "www.example.com", "api.example.com", "winner.com"}
...
func TestClassifyAcmeError_中文乱码时仍能归类(t *testing.T) {
	...
	got := classifyAcmeError(garbled, "manual-dns", "*.k8s-app-uat.com")
