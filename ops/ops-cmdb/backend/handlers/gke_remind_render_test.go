...
func TestRenderUpgradeRemind(t *testing.T) {
	items := []gkeRemindItem{
		...
		{
			Cluster: "app-prod-cluster", Env: "PROD",
	...
	// 1. 倒计时升序：最急的排最前
	iAPP := strings.Index(got, "app-prod-cluster")
	iInfra := strings.Index(got, "infra-k8s-cluster-01")
	if iAPP < 0 || iInfra < 0 || iAPP > iInfra {
	...
	if n := strings.Count(got, "app-prod-cluster"); n != 1 {
...
func TestRenderSkewOnly(t *testing.T) {
	got := renderUpgradeRemind([]gkeRemindItem{{
		Cluster: "staging-cluster-01", Env: "UAT",
