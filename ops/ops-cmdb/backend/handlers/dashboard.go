...
func (h *DashboardHandler) Get(c *gin.Context) {
	...
	resp := gin.H{
		...
		// 🔴 分母：**没探过**的有多少。
		//
		//	上面两个计数都带 `cert_expiry_at IS NOT NULL` —— 没探过的域名被整批排除在外。
		//	于是 828 条从没探测过的情况下，两个数都是 0，
		//	界面据此说「没有临期证书」——而那句话在这种情况下**不成立**：
		//	我们不知道它们什么时候到期（生产实测，OPSCMDB-063 / OPSCMDB-060）。
		//
		// ⚠️ 这个数必须**恒发**，0 也发：不发的话前端分不清
		//	"全都探过了且没问题"和"这一项根本没查"。
		"online_cert_unprobed": count("online_cert_unprobed", `SELECT COUNT(*) FROM domain_records WHERE ignored=0 AND cert_ignored=0 AND cert_expiry_at IS NULL`),
	}
	...
	// web-video.com 被算进了「30 天内到期」。负数天数必须单独归类——
	...
	if firstErr != nil {
		httpx.Fail(c, httpx.CodeInternal, fmt.Errorf("展示台统计查询失败: %w", firstErr), nil)
		return
	}
