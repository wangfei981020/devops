...
func NewGKEUpgradeHandler(st *store.Store, db *sql.DB) *GKEUpgradeHandler {
	return &GKEUpgradeHandler{Store: st, DB: db}
}
...
type gkeClusterOut struct {
	...
	// 实测 app：控制面 1.34（EOS 2027-01-25，绿色 178 天），但 35 个节点全在 1.33
...
func (h *GKEUpgradeHandler) ClearOverride(c *gin.Context) {
	...
	id, _ := strconv.Atoi(c.Param("id"))
	if _, err := sc.Exec(`UPDATE gke_version_schedule SET is_manual=0 WHERE tenant_id = ? AND id=?`, id); err != nil {
		c.JSON(http.StatusOK, gin.H{"ok": false, "error": err.Error()})
		return
	}
...
func classifyPause(reason string) (kind, note string) {
	...
	// 🔴 pausedReason 是**逗号分隔的多个枚举**，实测生产上出现过
	// `MAINTENANCE_WINDOW,CLUSTER_DISRUPTION_BUDGET_MINOR_UPGRADE`。
	//
	//	原来这里是一个 switch，第一条匹配就返回 —— 后面那些原因被静默丢掉。
	//	丢掉的可能正是要紧的那条：MAINTENANCE_WINDOW（常态节流）排在前面，
	//	而真正挡住升级的 MAINTENANCE_EXCLUSION_* 排在后面时，
	//	页面会说「常态，到窗口就会自动升级」——**与事实相反**
	//	（OPSCMDB-031 P1-20）。
	//
	//	所以逐条翻译、全部保留，kind 取其中最严重的那一档。
	...
	for _, p := range parts {
		...
		k, n := classifyOnePause(p)
		// excluded 比 throttled 严重，一旦出现就不能被后面的 throttled 盖掉
		if kind != "excluded" {
			kind = k
		}
		if !seen[n] {
	...
	return kind, strings.Join(notes, "；")
}

// classifyOnePause 翻译**单个**枚举值。
func classifyOnePause(r string) (kind, note string) {
	switch {
	...
	case strings.Contains(r, "MAINTENANCE_EXCLUSION"):
		// 新增的排除类型也当成"真正挡住" —— 这个方向猜错了只是多提醒一次，
		// 猜成 throttled 则会把"被挡住"说成"常态"
		return "excluded", "已设维护排除（" + r + "），升级被真正挡住"
