...
func (h *ObsQueryHandler) ResourceWaste(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"ok": false, "error": "取实际用量失败(Prometheus 不可用?): " + err.Error(), "error_key": "error.fetchUsageFailed"})
		return
	}
	if len(usage) == 0 {
		c.JSON(http.StatusOK, gin.H{"ok": false,
			"error": "没有取到任何 Pod 实际用量，无法计算浪费；请先确认该集群的 Prometheus 数据源已配置且可达", "error_key": "error.noPodUsage"})
		return
	}
