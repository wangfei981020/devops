...
// rebuildAutoRelationsCore 重建自动关系边，逐租户跑。
//
// 关系图是纯租户内的东西：A 租户的负载均衡不该和 B 租户的主机连线。
// 所以外层按租户循环，每轮只看得见一个租户的数据。
func rebuildAutoRelationsCore(ctx context.Context, st *store.Store, db *sql.DB, p ProgressFn) (string, []TaskFailure, bool) {
	var failures []TaskFailure
	sc, serr := st.Tenant(ctx)
	if serr != nil {
		return "取租户作用域失败：" + serr.Error(), nil, false
	}

	// ── 1. 给负载均衡补 CI 记录 ──
	lbCI, lbFail := ensureLBCIs(sc)
	if lbFail != nil {
	...
	// ── K8s 侧：域名 → 入口 → 服务 ──
	//
	//	🔴 补这一段之前，图谱里**全部的边都是 `loadbalancer --backs--> host`**，
	//	只有一种类型（OPSCMDB-031 P1-27）。所有路径长度都是 1，
	//	点开任何起点看到的都是"一个 LB 连着几台机器" ——
	//	回答不了"这个域名背后是什么"，而那正是拓扑图存在的理由。
	k8sEdges, k8sStats, k8sFails := collectK8sEdges(sc)
	edges = append(edges, k8sEdges...)
	failures = append(failures, k8sFails...)
	...
	// K8s 侧单独一句，含**已知缺口** —— 摘要只报"建了 N 条"会让人以为图是完整的
	msg += "。" + k8sEdgeSummary(k8sStats)
...
func ensureLBCIs(sc *store.Scoped) (map[string]int64, *TaskFailure) {
	out := map[string]int64{}
	rows, err := sc.Query(`SELECT cloud_account_id, project, name FROM cloud_loadbalancers WHERE tenant_id = ?`)
	if err != nil {
	...
	if erows, e := sc.Query(`SELECT id, name FROM cis WHERE tenant_id = ? AND type=?`, ciTypeLB); e == nil {
		for erows.Next() {
			var id int64
			var name string
	...
	for name, id := range existing {
		if !live[name] {
			if _, e := sc.Exec(`DELETE FROM cis WHERE tenant_id = ? AND id=? AND type=?`, id, ciTypeLB); e == nil {
