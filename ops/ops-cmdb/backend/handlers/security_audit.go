...
type secRow struct {
	...
	// OPSCMDB-032：root 判定拆成三态，见 k8ssource/podsecurity.go
	rootInit, rootUnknown bool
}
...
func judgeSecurity(r secRow) *secFinding {
	...
	bump := func(s string) {
		...
		// ⚠️ 传进来一个不在表里的等级时，rank[s] 取到零值 0，比较必然不成立，
		// 于是这次 bump **静默失效**——那条风险照样出现在清单里，却没有严重度，
		// 排序时沉到最底下。写这段时我自己就顺手写了个 "low"，编译通过、测试也不报，
		// 全靠翻这张表才发现。给未知等级兜一个最低有效级，别让它悄悄消失。
		if _, ok := rank[s]; !ok {
			s = "info"
		}
		if rank[s] > rank[sev] {
	...
	// ⚠️ 下面三条是**三件不同的事**，分开报（OPSCMDB-032）。
	// 原来它们被压成同一条「以 root 运行（未设置 runAsNonRoot）」，
	// 于是 UAT 168 条里 92 条是误报，非 root 整改的验收方法直接失效 ——
	// 改完一批数字不降，改过的和没改的在报告里长得一模一样。
	if r.runAsRoot {
		// 单独的 runAsRoot 极其普遍，报 high 会淹掉真问题；它的价值在于与其它风险叠加。
		f.Risks = append(f.Risks, "主容器以 root 运行（有效 runAsUser=0）")
		bump("medium")
	}
	if r.rootInit {
		// 主容器安全、init 是 root：restricted PSA 过不了，但主进程没有 root 权限。
		// 整改动作也不同（改 init 或改用 fsGroup/volume 权限），所以必须单独成条。
		f.Risks = append(f.Risks,
			"含 root 初始化容器：主容器不是 root，但 initContainer 以 uid 0 跑，restricted PSA 不通过")
		bump("info")
	}
	if r.rootUnknown {
		// ⚠️ 判不了就说判不了。未声明 ≠ 以 root 跑 —— 实际 uid 由镜像 USER 决定，
		// 而 CMDB 采不到镜像内容。猜一个方向再当成事实，正是这个 bug 的根源。
		f.Risks = append(f.Risks,
			"运行用户未知：Pod 与容器两级都没声明 runAsUser/runAsNonRoot，实际 uid 取决于镜像 USER，需 docker inspect 或看 Dockerfile 确认")
		bump("info")
	}
