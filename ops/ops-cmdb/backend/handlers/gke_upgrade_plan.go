...
func (h *GKEUpgradePlanHandler) SaveBaseline(c *gin.Context) {
	...
	if err != nil {
		httpx.Fail(c, httpx.CodeInternal, fmt.Errorf("序列化基线失败: %w", err), nil)
		return
	}
	...
	if err != nil {
		httpx.Fail(c, httpx.CodeInternal, fmt.Errorf("保存基线失败: %w", err), nil)
		return
	}
...
func (h *GKEUpgradePlanHandler) ListBaselines(c *gin.Context) {
	...
	if err != nil {
		httpx.Fail(c, httpx.CodeInternal, fmt.Errorf("查询基线失败: %w", err), nil)
		return
	}
	defer rows.Close()
	out := []gin.H{}
...
func (h *GKEUpgradePlanHandler) AvailableVersions(c *gin.Context) {
	...
	if err != nil {
		httpx.Fail(c, httpx.CodeInternal, fmt.Errorf("查询可用版本失败: %w", err), nil)
		return
	}
	defer rows.Close()
	out := []string{}
...
func (h *GKEUpgradePlanHandler) Plan(c *gin.Context) {
	...
	if err != nil {
		...
		httpx.Fail(c, httpx.CodeInternal, fmt.Errorf("读取集群升级状态失败: %w", err), nil)
		return
	}
	...
	if st == nil {
		httpx.FailKey(c, httpx.CodeNotFound, "error.clusterOrGkeInfoMissing", nil, nil)
		return
	}
