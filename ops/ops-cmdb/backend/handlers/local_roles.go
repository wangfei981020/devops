...
const (
	...
	roleCost    = "cmdb_cost"
	// roleDiagnoser 专门给 AI / MCP 用：**能查一切、能诊断、什么都改不了**。
	// 与 cmdb_viewer 的区别是它额外拿到 read:* 档 —— 也就是日志、事件、
	// 规则诊断、自助查询这些"只读但要授权"的能力。
	roleDiagnoser = "cmdb_diagnoser"
)
...
// allReadCodes 是**只读能力**码：只读，但直连集群/外部数据源、可能触达敏感内容
// （日志里有业务数据和连接串），所以要显式授权 —— 但它们一个字都不写。
//
//	⚠️ 这一档是补出来的。原来只有 menu:*（能看哪页）和 cmdb:*（能做什么），
//	于是"读日志""跑诊断"只能挂到动作码上，而只读角色按定义不拿动作码 ——
//	结果 diagnose_pod / pod_logs 这些 MCP 工具对只读令牌完全不可见，
//	把「只读令牌完成诊断」这个产品目标直接堵死了（OPSCMDB-024）。
//
//	判据是**会不会改东西**，不是**要不要授权**。
var allReadCodes = []string{
	"read:k8s_diag",   // 规则诊断：根因 + 证据 + 处置建议
	"read:pod_logs",   // Pod 日志与事件（经 apiserver 代理到 kubelet）
	"read:prom_query", // Prometheus 自助查询
	"read:loki",       // Loki 日志检索
}
...
func builtinLocalRoles() []localRoleSeed {
	return []localRoleSeed{
		...
		// ⚠️ 只读诊断：**全部 menu + 全部 read，零个 cmdb:***。
		// 它比 cmdb_viewer 多的正是诊断能力，比任何管理角色少的是全部写操作。
		// MCP 令牌应该默认绑这个，而不是 cmdb_viewer（那个连日志都读不到）
		// 或管理员（那个能改线上）。
		{roleDiagnoser, "CMDB 只读诊断",
			"查得到全部台账，还能读日志/事件、跑规则诊断、自助查指标——但改不了任何东西。给 AI 接入用的角色。",
			withParent(allMenuCodes, allReadCodes)},
...
func (h *LocalRoleHandler) List(c *gin.Context) {
	...
	if err != nil {
		httpx.Fail(c, httpx.CodeInternal, fmt.Errorf("读取角色失败: %w", err), nil)
		return
	}
	defer rows.Close()
	...
	type item struct {
		...
		// Perms 这个角色具体的权限码。
		//
		//	🔴 只给 perm_count 的后果：页面标题写着「用户与角色」，
		//	但**看不到任何一个角色具体能做什么** ——「CMDB 资产管理员」
		//	和「CMDB 集群管理员」的区别界面上无从得知，分配角色只能靠名字猜
		//	（OPSCMDB-031 P1-67）。数字回答不了"能做什么"这个问题。
		//
		//	⚠️ 只给码不给中文名：码到可读名的映射在前端（nav.ts 是菜单的
		//	唯一数据源，权限码就是从那儿来的）。后端再写一份必然分叉。
	...
	for rows.Next() {
		...
		// 不能是 nil —— JSON 里 null 和 [] 在前端是两件事：
		// 前者会被读成"这个角色的权限没取到"，后者才是"确实一条都没有"
		it.Perms = list
		if it.Perms == nil {
			it.Perms = []string{}
		}
