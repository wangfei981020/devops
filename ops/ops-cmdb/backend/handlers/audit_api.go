...
func NewAuditHandler(st *store.Store, db *sql.DB) *AuditHandler {
	return &AuditHandler{Store: st, DB: db}
}
...
func (h *AuditHandler) List(c *gin.Context) {
	...
	// ⚠️ 同时接受 `size` 和 `page_size`。
	//
	//	本产品其它列表页的分页参数一律叫 `size`，只有这里叫 `page_size` ——
	//	于是拿 `size=100` 去调这个接口会**静默返回 10 条**，
	//	看起来像"分页大小不生效"（OPSCMDB-031 P0-23 就是这么测出来的）。
	//	参数名不统一而又有默认值兜底，正是那种不报错、只是答非所问的坑。
	//
	//	`size` 优先（那是全站的正名），`page_size` 保留兼容已有调用方。
	...
	//
	// 🔴 不要让前端算好绝对时间再传过来。
	//	`at` 是 DATETIME，而 DSN 里 time_zone='+08:00'（见 config.buildDSN），
	//	也就是说库里存的是**马尼拉时间**。浏览器发 ISO-8601 的 UTC 串
	//	（`2026-08-19T02:00:00.000Z`）会同时踩两个坑：
	//	  ① MySQL 解析不了尾巴上的 `.000Z`，只会给个截断警告
	//	  ② 就算解析了，UTC 与库里的 +08:00 差 8 小时
	//	两个坑都**不报错**，只是把「近 6 小时」悄悄变成另一个时间窗。
	//	用 NOW() 在库里算，会话时区和列的时区天然一致，全链路不做任何换算。
	if v := c.Query("hours"); v != "" {
		if n, e := strconv.Atoi(v); e == nil && n > 0 {
			add("at >= DATE_SUB(NOW(), INTERVAL ? HOUR)", n)
		} else {
			// 传了但解析不出来 = 请求方以为在筛、实际没筛。必须说出来，
			// 否则「近 6 小时」会返回全量而看着像"这 6 小时内发生了很多事"
			logx.J("audit", "bad_hours_param", map[string]any{
				"hours": v, "warn": "hours 不是正整数，本次未按时间窗过滤",
			})
		}
	}
	// since/until 保留：给按绝对时间点查的场景（对接方/脚本）。
	// ⚠️ 传的必须是 `YYYY-MM-DD HH:MM:SS` 且为 UTC+8，理由同上。
	...
	c.JSON(200, gin.H{"total": total, "page": page, "page_size": size, "size": size, "list": list})
}
...
func (h *AuditHandler) RevertChange(c *gin.Context) {
	...
	if err != nil {
		httpx.NotFound(c, "audit_change")
		return
	}
	if ch.RevertedAt.Valid {
		httpx.FailKey(c, httpx.CodeConflict, "error.changeAlreadyRolledBack", nil, nil)
		return
	}
	...
	if ch.RevertKind == "external" {
		...
		// ⚠️ 中文那两句留着是给 MCP / 直接调 API 的人看的（他们读不到语言包），
		//	界面走 message_key + hint_key（OPSCMDB-054）
		httpx.FailKeyWith(c, httpx.CodeBadRequest, "error.revertExternalUnsupported", nil,
			map[string]any{
				"hint_key": "error.revertExternalHint",
	...
	if ch.Op != "INSERT" || after != nil {
		...
		if conflict, fields := detectConflict(after, cur); conflict && !body.Force {
			...
			httpx.FailKeyWith(c, httpx.CodeConflict, "error.revertConflict", nil,
				map[string]any{
					"hint_key":        "error.revertConflictHint",
	...
	case "INSERT":
		...
		if _, err := h.DB.Exec(fmt.Sprintf("DELETE FROM `%s` WHERE `%s`=?", ch.Table, pkCol), ch.PK); err != nil {
			httpx.Fail(c, httpx.CodeInternal, fmt.Errorf("回滚失败: %w", err), nil)
			return
		}
		opDone = "DELETE"
	case "DELETE":
		...
		if errB != nil || before == nil {
			httpx.FailKey(c, httpx.CodeBadRequest, "error.noBeforeSnapshotDelete", nil, nil)
			return
		}
		if err := insertRow(h.DB, ch.Table, before); err != nil {
			httpx.Fail(c, httpx.CodeInternal, fmt.Errorf("恢复失败: %w", err), nil)
			return
		}
		opDone = "INSERT"
	default: // UPDATE
		if errB != nil || before == nil {
			httpx.FailKey(c, httpx.CodeBadRequest, "error.noBeforeSnapshot", nil, nil)
			return
		}
		if err := updateRow(h.DB, ch.Table, pkCol, ch.PK, before); err != nil {
			httpx.Fail(c, httpx.CodeInternal, fmt.Errorf("还原失败: %w", err), nil)
			return
		}
		opDone = "UPDATE"
	}
	...
	if _, err := sc.Exec(`UPDATE audit_changes SET reverted_at=NOW(), reverted_by=?, revert_audit_id=? WHERE tenant_id = ? AND id=?`,
		UsernameFromCtx(c), newAuditID, ch.ID); err != nil {
		// 回滚已经做了，只是标记没写上 —— 必须报出来，
		// 否则这条记录会显示成"可回滚"，有人再点一次就是重复回滚。
		logx.Line("audit", fmt.Sprintf("revert 标记失败 change=%d: %v", ch.ID, err))
	}
...
func (h *AuditHandler) RevertAll(c *gin.Context) {
	...
	rows.Close()
	if len(ids) == 0 {
		httpx.FailKey(c, httpx.CodeBadRequest, "error.nothingToRollback", nil, nil)
		return
	}
	c.JSON(200, gin.H{"ok": true, "change_ids": ids,
		"hint_key": "error.revertOneByOne",
