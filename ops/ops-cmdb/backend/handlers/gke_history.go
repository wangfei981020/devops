...
func (h *GKEHistoryHandler) UpgradeHistory(c *gin.Context) {
	// ⚠️ 两个值都取：cluster 装技术名、cluster_display 装别名。
	//	拍成一个 COALESCE 的话前端就再也拿不到技术名了（OPSCMDB-078）。
	q := `SELECT h.cluster_id, COALESCE(c.name, ''), COALESCE(c.display_name, c.name, ''), h.scope, h.pool,
	...
	for rows.Next() {
		var cid int
		var cname, cdisp, scope, pool, st, state, iv, tv, detail, src string
		var startedAt, endedAt sql.NullString
		if rows.Scan(&cid, &cname, &cdisp, &scope, &pool, &st, &state, &iv, &tv,
		...
		out = append(out, gin.H{
			"cluster_id": cid, "cluster": cname, "cluster_display": cdisp, "scope": scope, "pool": pool,
...
func (h *GKEHistoryHandler) RepairHistory(c *gin.Context) {
	// ⚠️ 两个值都取：cluster 装技术名、cluster_display 装别名。
	//	拍成一个 COALESCE 的话前端就再也拿不到技术名了（OPSCMDB-078）。
	q := `SELECT r.cluster_id, COALESCE(cl.name, ''), COALESCE(cl.display_name, cl.name, ''), r.op_name, r.pool, r.node_name,
	...
	for rows.Next() {
		var cid int
		var cname, cdisp, op, pool, node, reason, status, detail, msg string
		var startedAt, endedAt sql.NullString
		if rows.Scan(&cid, &cname, &cdisp, &op, &pool, &node, &reason, &status,
		...
		out = append(out, gin.H{
			"cluster_id": cid, "cluster": cname, "cluster_display": cdisp, "op_name": op, "pool": pool, "node_name": node,
