...
// maxDNSBatch 一次批量操作的行数上限。
//
// ⚠️ 这个数字**同时出现在校验和文案里**（"一次最多 N 条"）。
//
//	原来是三处写死的 200 加三句写死的中文 —— 改上限时必然漏掉某一处，
//	而漏掉的那一处不报错，只会让提示和实际校验对不上。
const maxDNSBatch = 200
...
func (h *SyncHandler) CreateDNSRecord(c *gin.Context) {
	...
	if err != nil {
		httpx.FailKey(c, httpx.CodeUpstreamError, "error.dnsReadRecordsFailed", err, nil)
		return
	}
	for _, r := range group {
		if r.Data == strings.TrimSpace(in.Data) {
			httpx.FailKey(c, httpx.CodeConflict, "error.dnsRecordDuplicate", nil, nil)
			return
		}
	}
	...
	if err := wa.AddRecords(ctx, domain, []dnsource.DNSRecord{newRec}); err != nil {
		httpx.FailKey(c, httpx.CodeUpstreamError, "error.dnsWriteBackFailed", err, nil)
		return
	}
	ciID, _ := parseID(ciid)
	if !wa.DryRun() {
...
func (h *SyncHandler) BatchCreateDNSRecord(c *gin.Context) {
	...
	if len(in.Records) == 0 {
		httpx.FailKey(c, httpx.CodeBadRequest, "error.dnsNothingToAdd", nil, nil)
		return
	}
	if len(in.Records) > maxDNSBatch {
		httpx.FailKey(c, httpx.CodeBadRequest, "error.dnsBatchLimit", nil, map[string]any{"max": maxDNSBatch})
		return
	}
	...
	if len(valid) == 0 {
		httpx.FailKeyWith(c, httpx.CodeBadRequest, "error.dnsAllRowsInvalid", nil, map[string]any{"errors": errs})
		return
	}
	wa, ad, domain, sourceID, err := h.writeAdapterForDomain(ciid)
	if err != nil {
	...
	c.JSON(200, gin.H{"ok": true, "dry_run": wa.DryRun(), "env": wa.EnvLabel(),
		...
		// ⚠️ ok_count / fail_count 是**跨三个批量接口统一**的计数字段。
		// 原来只有各自的 added/skipped、deleted/skipped、updated/failed ——
		// 三个语义相同的接口三套名字，前端只能读一套，结果三个都取不到，
		// 「部分成功」在界面上看着和全成功一样。原字段名保留，不破坏已有调用方。
		"ok_count": len(valid), "fail_count": len(errs),
...
func (h *SyncHandler) BatchDeleteDNSRecords(c *gin.Context) {
	...
	if len(in.IDs) == 0 {
		httpx.FailKey(c, httpx.CodeBadRequest, "error.dnsNothingToDelete", nil, nil)
		return
	}
	if len(in.IDs) > maxDNSBatch {
		httpx.FailKey(c, httpx.CodeBadRequest, "error.dnsBatchLimit", nil, map[string]any{"max": maxDNSBatch})
		return
	}
	ciID, _ := parseID(ciid)
	...
	if len(toDel) == 0 {
		httpx.FailKeyWith(c, httpx.CodeBadRequest, "error.dnsNoDeletable", nil, map[string]any{"errors": errs})
		return
	}
	wa, ad, domain, sourceID, err := h.writeAdapterForDomain(ciid)
	if err != nil {
	...
	c.JSON(200, gin.H{"ok": true, "dry_run": wa.DryRun(), "env": wa.EnvLabel(),
		...
		"ok_count": deleted, "fail_count": len(errs),
...
func (h *SyncHandler) BatchUpdateDNSRecords(c *gin.Context) {
	...
	if len(in.Records) == 0 {
		httpx.FailKey(c, httpx.CodeBadRequest, "error.dnsNothingToEdit", nil, nil)
		return
	}
	if len(in.Records) > maxDNSBatch {
		httpx.FailKey(c, httpx.CodeBadRequest, "error.dnsBatchLimit", nil, map[string]any{"max": maxDNSBatch})
	...
	if len(edits) == 0 {
		httpx.FailKeyWith(c, httpx.CodeBadRequest, "error.dnsNoEditable", nil, map[string]any{"errors": errs})
		return
	}
	wa, ad, domain, sourceID, err := h.writeAdapterForDomain(ciid)
	if err != nil {
	...
	c.JSON(200, gin.H{"ok": true, "dry_run": wa.DryRun(), "env": wa.EnvLabel(),
		...
		"ok_count": updated, "fail_count": len(errs),
...
func (h *SyncHandler) UpdateDNSRecord(c *gin.Context) {
	...
	if !replaced {
		httpx.FailKey(c, httpx.CodeConflict, "error.dnsRecordChangedUpstream", nil, nil)
		return
	}
...
func (h *SyncHandler) DeleteDNSRecord(c *gin.Context) {
	...
	if isProtectedTypeName(rtype, name) {
		httpx.FailKey(c, httpx.CodeForbidden, "error.dnsRecordProtected", nil, nil)
		return
	}
	wa, ad, domain, sourceID, err := h.writeAdapterForDomain(fmt.Sprint(domCiID))
	if err != nil {
