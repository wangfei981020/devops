...
type K8sCostHandler struct {
	Store *store.Store
	DB    *sql.DB
}

func NewK8sCostHandler(st *store.Store, db *sql.DB) *K8sCostHandler {
	return &K8sCostHandler{Store: st, DB: db}
}
...
func (h *K8sCostHandler) buildItems(sc *store.Scoped) []costItem {
	rc := newRateCache(sc)
	cls := h.clusters()
	nsp := h.nsProjects()
...
func (h *K8sCostHandler) Detail(c *gin.Context) {
	...
	items := h.filtered(h.buildItems(sc), c)
	var total float64
	for _, it := range items {
		total += it.Cost
	}
...
func (h *K8sCostHandler) SetNodeOverride(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&in); err != nil || in.ClusterID == 0 || in.Name == "" {
		httpx.Required(c, "cluster_id/name")
		return
	}
