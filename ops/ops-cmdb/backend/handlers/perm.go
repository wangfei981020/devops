...
var permPrefixRules = []permRule{
	// 资产：域名 / DNS / 证书 / CDN
	{"/api/domains/quality", "menu:cmdb_domains", ""}, // 只读对账，与域名列表同权限
	{"/api/domains", "menu:cmdb_domains", "cmdb:manage_domains"},
	{"/api/domain-list", "menu:cmdb_domains", ""},
	...
	{"/api/registrar/dns-records", "menu:cmdb_dns_records", ""},
	{"/api/certs", "menu:cmdb_certs", "cmdb:manage_certs"},
	{"/api/cert-list", "menu:cmdb_certs", ""},
	...
	// 云资源
	{"/api/overview", "menu:cmdb_overview", ""},
	// 分诊与全局态势同档：同一套判据的两种呈现（一个给人看，一个给 AI 看）
	{"/api/triage", "menu:cmdb_overview", ""},
	{"/api/task-list", "menu:cmdb_cron", ""},
	{"/api/cost/overview", "menu:cmdb_cost", ""},
	{"/api/datasource-list", "menu:cmdb_integrations", ""},
	{"/api/exposure-list", "menu:cmdb_k8s_networking", ""},
	{"/api/relation-list", "menu:cmdb_relations", ""},
	{"/api/k8s/event-list", "menu:cmdb_k8s_events", ""},
	{"/api/impact", "menu:cmdb_k8s_topology", ""},
	...
	{"/api/k8s/cluster-list", "menu:cmdb_k8s_clusters", ""},
	{"/api/k8s/sync-state", "menu:cmdb_k8s_clusters", ""},
	{"/api/k8s/nodes", "menu:cmdb_k8s_nodes", ""},
	{"/api/k8s/node-list", "menu:cmdb_k8s_nodes", ""},
	...
	{"/api/k8s/pvc-list", "menu:cmdb_k8s_storage", ""},
	...
	// 体检项下钻：纯只读，能看体检页就该看得到明细
	{"/api/k8s/health/detail", "menu:cmdb_k8s_health", ""},
	...
	{"/api/k8s/namespace-list", "menu:cmdb_k8s_ns_project", ""},
	...
	// 所以读也要显式授权，不是给了菜单就能拉日志。
	//
	// ⚠️ 用 `read:*` 而不是 `cmdb:*`。原来挂在 cmdb:k8s_diag（**动作码**）上，
	// 而只读角色按定义只拿菜单码 —— 于是 diagnose_pod / pod_logs / pod_events
	// 这三个 MCP 工具对只读令牌**完全不可见**，而产品目标恰恰是
	// 「只读令牌完成发现→分析→方案」（OPSCMDB-024）。
	//
	// 分类的判据是**会不会改东西**，不是**要不要授权**：
	//   menu:*  能看哪一页
	//   read:*  只读，但直连外部/可能触敏，需显式授权   ← 诊断属于这一档
	//   cmdb:*  会改东西
	//
	// 逗号 = 任一命中即可，所以旧的 cmdb:k8s_diag 仍然放行，老角色不受影响。
	{"/api/k8s/diagnose", "read:k8s_diag,cmdb:k8s_diag", ""},
	{"/api/k8s/diagnose-cluster", "read:k8s_diag,cmdb:k8s_diag", ""},
	{"/api/k8s/diagnose-sweep", "read:k8s_diag,cmdb:k8s_diag", ""},
	{"/api/diagnose-domain", "read:k8s_diag,cmdb:k8s_diag", ""},
	{"/api/diagnose-cost", "read:k8s_diag,cmdb:k8s_diag", ""},
	{"/api/k8s/pod-logs", "read:pod_logs,cmdb:k8s_diag", ""},
	{"/api/k8s/pod-events", "read:pod_logs,cmdb:k8s_diag", ""},
	// 容器名单是取日志的前置步骤，权限跟日志一档（能看日志才能知道有哪些容器）
	{"/api/k8s/pod-containers", "read:pod_logs,cmdb:k8s_diag", ""},
	// 自助查询类：同样是只读但直连外部数据源
	{"/api/obs/loki", "read:loki,menu:cmdb_k8s_usage", ""},
	{"/api/obs/prom-query", "read:prom_query,menu:cmdb_k8s_usage", ""},
	{"/api/obs/prom-labels", "read:prom_query,menu:cmdb_k8s_usage", ""},
	{"/api/obs/prom-metrics", "read:prom_query,menu:cmdb_k8s_usage", ""},
	...
	// 探测标签名：只读，给「集群隔离标签」下拉用。看得到接入页就用得了
	{"/api/obs-endpoints/:id/label-names", "menu:cmdb_integrations", ""},
	...
	// SSO 接入跟用户管理同一个权限码：能管账号的人才谈得上改"账号从哪来"。
	// ⚠️ 登录链路本身（/api/auth/sso/*）注册在鉴权中间件之前，不走这张表——
	// 用户就是因为还没登录才走那条路
	{"/api/idp-config", "menu:cmdb_users", "cmdb:manage_users"},
...
var permExactRules = map[string]string{
	...
	// 激活授权是管理动作，挂基础配置的**操作**权限。
	//
	// 🔴 这里原来写的是 `menu:cmdb_basic` —— 那是"能不能看见这个菜单"，
	//	不是"能不能执行这个动作"。只读角色也有菜单权限，于是它能换掉
	//	整个系统的授权（实测 cmdb_viewer 打 POST /api/license 进到了激活流程，
	//	返回的是"激活码无效"而不是 403）。
	//	判据要用 cmdb:manage_*，跟同一张表里其他写路由一致。
	// 注意它同时在 license.alwaysWritable 里 —— 只读降级下必须仍能激活，
	// 否则过期之后连自救的路都没有了。
	//	⚠️ 那是"只读降级"（授权过期导致的全局降级），
	//	与"这个账号是只读角色"是两回事，别混为一谈：前者要放行，后者要拒绝。
	"POST /api/license": "cmdb:manage_basic",
}
...
var permPublicRead = map[string]bool{
	...
	// 授权状态：**登录即可读**。
	//
	// 过期只读时，界面要给每个人一条横幅说明"为什么保存按钮没反应"。
	// 挂上管理权限的话，普通用户看到的就只是一堆莫名失败的写操作 ——
	// 而他会以为是系统坏了，然后来报障。
	//
	// 里面的安装指纹不是凭据：它只是个标识，解锁还需要我们的私钥签名，
	// 且客户本来就要把它报给我们才能签发。
	"GET /api/license": true,
