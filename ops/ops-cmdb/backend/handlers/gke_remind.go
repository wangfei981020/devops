...
// 之前这里只 JOIN gke_cluster_upgrade、没碰 gke_node_pools，导致三天后 35 个节点进 EOS
...
func buildRemindItem(s *clusterUpgradeState, today time.Time) gkeRemindItem {
	...
	// 生产上 5 个池里 4 个是这种，每次都跟着刷但不需要人动手（分档处理）
	if s.SkewCritical {
...
func sendUpgradeRemind(db *sql.DB, items []gkeRemindItem) string {
	...
	if webhook == "" {
		...
		return "没有可用的飞书出口（未绑群且无全局兜底），提醒未送达"
	}
