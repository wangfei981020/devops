...
func (h *K8sResourceHandler) ListOrphans(c *gin.Context) {
	...
	if kind == "" || kind == "pvc" {
		items = append(items, h.orphanPVCs(sc, cid)...)
	}
...
func (h *K8sResourceHandler) orphanPVCs(sc *store.Scoped, cid string) []orphanItem {
...
func (h *K8sResourceHandler) emptyNamespaces(cid string) []orphanItem {
	...
	for rows.Next() {
		...
		if why := onDemandNamespace(name); why != "" {
			// 🔴 按需才有 Pod 的命名空间不算孤儿。删了会真出事，见 onDemandNamespace 的说明
			logx.D("orphans", "skip_on_demand_ns", map[string]any{
				"cluster_id": cid, "namespace": name, "reason": why, "configmaps": cmCount,
			})
			continue
		}
		it := orphanItem{
		...
		switch {
		case cmCount >= configHeavyNamespaceCM:
			// 🔴 装了大量配置的命名空间**不给删除命令**，只给"去看一眼"。
			//
			//	这是 031 建议的第三条判据（前两条是名字模式和平台白名单，
			//	都已在 onDemandNamespace 里）。它兜住的是**名字看不出来**的那些：
			//	一个不叫 *-devops、也不在白名单里、但装着几百个 ConfigMap 的命名空间，
			//	几乎不可能是"业务遗留的空壳"——它在承载配置，只是不跑常驻 Pod。
			//
			//	⚠️ 这里刻意**不给 delete 命令**，而不是"给命令 + 加警告"。
			//	原实现是后者，而 031 的判断是对的：
			//	**警告不能替代判据** —— 一个带着删除命令、出现在「孤儿资源」清单里的条目，
			//	读的人默认它是可以删的。
			it.Reason = "没有 Pod，但装着大量配置——需人工确认"
			it.Detail = fmt.Sprintf("里面有 %d 个 ConfigMap。没有常驻 Pod 但配置这么多，"+
				"更像是「按需才起 Pod」的命名空间（CI/CD agent、Job 专用、外部系统的项目空间），"+
				"而不是业务遗留的空壳。删除前必须搞清楚这些配置是谁在用。", cmCount)
			it.Action = fmt.Sprintf("先看里面装了什么：kubectl -n %s get configmap,secret,rolebinding,networkpolicy"+
				"；确认真的没人用之后再考虑删除（本条不给删除命令是刻意的）", name)
		case cmCount > 0:
...
// configHeavyNamespaceCM 「装着大量配置」的 ConfigMap 数量门槛。
//
//	取 10：k8s 每个命名空间默认自带 1 个 kube-root-ca.crt，
//	所以 0~1 个才是真的空；到 10 个说明有人在往里放东西。
//	实测那几个 KubeSphere DevOps 项目是 132 / 608 / 666 个，远超这个线。
//
//	⚠️ 阈值宁可低（宽松地判成"需确认"），不要高。
//	判错的两个方向代价完全不对称：
//	  多要一次人工确认  →  多花两分钟
//	  漏掉一个配置命名空间 → 照着删除命令执行，搞坏整个构建体系
const configHeavyNamespaceCM = 10

// isSystemNamespace 天生可能为空的系统命名空间，不该报为孤儿。
// onDemandNamespace 判断这个命名空间是不是「按需才有 Pod」的那一类。
// 返回非空字符串表示是，内容为原因。
//
// 🔴 为什么必须有这个判定
//
// 空命名空间的判据是「没有工作负载、也没有 Pod」。但有整整一类命名空间
// **平时本来就是 0 个 Pod**，只在需要时才起：
//
//   - CI/CD agent 命名空间：Jenkins/Tekton 的 agent Pod 是构建时才创建、构建完就销毁
//   - KubeSphere DevOps 项目：流水线定义存在 ConfigMap/CRD 里，根本不跑常驻 Pod
//   - 平台组件命名空间：Harbor、ArgoCD、KubeSphere 控制面等
//
// 生产实测（DEV 集群）：53 个「孤儿命名空间」里有大量 `*-devops*`，
// 其中 g32-uat-devopsrkf2b 装着 **666 个 ConfigMap**，
// kubesphere-devops-worker 是 Jenkins agent 的落点。
// 而每条都配着 `kubectl delete ns`——**照做会直接搞坏整个构建体系**。
//
// ⚠️ 原实现已经在 Detail 里警告了「删 ns 会一并删掉 N 个 ConfigMap」，
// 但仍把它们列进「孤儿资源」并给了删除命令。
// **警告不能替代判据**：一个带着删除命令、出现在「孤儿/浪费」清单里的条目，
// 读的人默认它是可以删的。
func onDemandNamespace(ns string) string {
	// KubeSphere DevOps 项目：创建时按 <workspace>-<project>-devops<随机后缀> 命名
	if strings.Contains(ns, "-devops") || strings.HasSuffix(ns, "devops") {
		return "KubeSphere DevOps 项目：流水线定义存在 ConfigMap/CRD 里，平时没有常驻 Pod"
	}
	// 平台组件：这些命名空间即使暂时没 Pod 也绝不该被当成可回收资源
	for _, p := range []string{
		"kubesphere-", "harbor", "argocd", "istio-", "cert-manager",
		"velero", "logging", "loki", "monitoring", "extension-",
	} {
		if ns == strings.TrimSuffix(p, "-") || strings.HasPrefix(ns, p) {
			return "平台组件命名空间，不是业务遗留"
		}
	}
	return ""
}
