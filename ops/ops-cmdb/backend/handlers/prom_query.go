...
func (h promQueryHandler) Query(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"ok": false, "error": "连接 Prometheus 失败: " + err.Error(), "error_key": "error.connectPrometheusFailed"})
		return
	}
	if code != 200 {
	...
	if err := json.Unmarshal([]byte(body), &r); err != nil {
		c.JSON(http.StatusOK, gin.H{"ok": false, "error": "解析响应失败: " + err.Error(), "error_key": "error.parseResponseFailed"})
		return
	}
	if r.Status != "success" {
...
func (h promQueryHandler) Metrics(c *gin.Context) {
	...
	if err != nil || code != 200 {
		c.JSON(http.StatusOK, gin.H{"ok": false, "error": fmt.Sprintf("取指标列表失败 (HTTP %d): %v", code, err), "error_key": "error.promListMetricsFailed", "error_params": gin.H{"status": code}})
		return
	}
	var r struct {
	...
	if json.Unmarshal([]byte(body), &r) != nil {
		c.JSON(http.StatusOK, gin.H{"ok": false, "error": "解析指标列表失败", "error_key": "error.promParseMetricsFailed"})
		return
	}
...
func (h promQueryHandler) Labels(c *gin.Context) {
	...
	code, body, err := obsGet(u, token, 30*time.Second)
	if err != nil || code != 200 {
		c.JSON(http.StatusOK, gin.H{"ok": false, "error": fmt.Sprintf("取标签值失败 (HTTP %d): %v", code, err), "error_key": "error.promListLabelValuesFailed", "error_params": gin.H{"status": code}})
		return
	}
	var r struct {
		Data []string `json:"data"`
	...
	if json.Unmarshal([]byte(body), &r) != nil {
		c.JSON(http.StatusOK, gin.H{"ok": false, "error": "解析标签值失败", "error_key": "error.promParseLabelValuesFailed"})
		return
	}
	sort.Strings(r.Data)
	// 🔴 截断必须说出来，而且 count 要报**真实总数**。
	//
	//	原来是 `r.Data = r.Data[:500]` 之后 `count: len(r.Data)` ——
	//	于是一个有 3000 个取值的标签，返回 `count: 500` 且没有任何标记，
	//	读的人只能得出"这个标签一共 500 个值"这个错误结论，
	//	然后按它去写查询、去估规模。
	//
	//	⚠️ 静默截断是本项目反复在修的那一类：结果看起来完全正常。
	total := len(r.Data)
	out := gin.H{"ok": true, "label": label, "count": total}
	if total > 500 {
		r.Data = r.Data[:500]
		out["truncated"] = fmt.Sprintf(
			"该标签共有 %d 个取值，这里只返回了前 500 个（按字典序）。"+
				"要找特定的值请用 metric 参数收窄，或直接在查询里写完整匹配", total)
	}
	out["values"] = r.Data
	c.JSON(http.StatusOK, out)
}
