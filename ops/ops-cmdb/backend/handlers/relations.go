...
func NewRelationHandler(st *store.Store, db *sql.DB) *RelationHandler {
	return &RelationHandler{Store: st, DB: db}
}
...
func (h *RelationHandler) Delete(c *gin.Context) {
	...
	if _, err := sc.Exec(`DELETE FROM ci_relations WHERE tenant_id = ? AND id=?`, c.Param("id")); err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
		return
	}
