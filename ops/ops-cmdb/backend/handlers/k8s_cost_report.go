...
func (h *K8sCostHandler) snapshotRows(sc *store.Scoped) []snapRow {
	...
	for _, it := range h.buildItems(sc) {
...
func (h *K8sCostHandler) takeSnapshot(sc *store.Scoped, month string) (int, error) {
	rows := h.snapshotRows(sc)
	// ⚠️ 这条 DELETE 原来是 `WHERE month=?` —— 多租户下任一租户生成快照，
	//    会把**所有租户**当月的快照一起删掉，然后只写回自己的。
	//    别人的历史成本数据就这么没了，且不会有任何报错。
	if _, err := sc.Exec(`DELETE FROM cost_snapshots WHERE tenant_id = ? AND month=?`, month); err != nil {
		return 0, err
	}
	for _, r := range rows {
		_, _ = sc.Insert(`INSERT INTO cost_snapshots (tenant_id,month,cluster,mode,gcp_project,biz_project,env,type,resource_key,spec,cost)
			VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
...
func (h *K8sCostHandler) Report(c *gin.Context) {
	...
	if anchor == "" {
		// month 是历史别名。前端一度传的就是它，而这里只读 anchor ——
		// 结果是选哪个月都返回当前月（OPSCMDB-049）。
		// 前端已改正，这里认下这个名字是给 MCP 和外部调用方兜底：
		// 静默忽略一个明明传了的参数，比报错难查得多。
		anchor = c.Query("month")
	}
	if anchor == "" {
		anchor = time.Now().Format("2006-01")
	}
	// ⚠️ 格式不对时不能悄悄退回当前月 —— 那又是一次"答非所问"。
	if _, err := time.Parse("2006-01", anchor); err != nil {
		httpx.Invalid(c, "anchor", "YYYY-MM")
		return
	}
...
func (h *K8sCostHandler) Attribution(c *gin.Context) {
	...
	// 🔴 按维度聚合。
	//
	//	dim 原来**根本没读** —— 而前端的「维度」下拉一直在传它。
	//	用户在项目/集群/类型之间切换，结果一动不动，
	//	看起来就像"这三个维度下涨的都是同一批东西"（OPSCMDB-049）。
	//	默认 resource 保持原有行为（资源级明细），选了维度才聚合。
	switch dim := c.Query("dim"); dim {
	case "", "resource":
		// 资源级：不聚合
	case "project", "cluster", "type":
		key := map[string]string{"project": "project", "cluster": "cluster", "type": "type"}[dim]
		agg := map[string]gin.H{}
		order := []string{}
		for _, m := range movers {
			...
			g, ok := agg[k]
			if !ok {
				g = gin.H{"resource": k, key: k, "old": 0.0, "new": 0.0, "delta": 0.0, "n": 0}
				agg[k] = g
				order = append(order, k)
			}
			g["old"] = round2(g["old"].(float64) + m["old"].(float64))
			g["new"] = round2(g["new"].(float64) + m["new"].(float64))
			g["delta"] = round2(g["delta"].(float64) + m["delta"].(float64))
			g["n"] = g["n"].(int) + 1
		}
		agged := make([]gin.H, 0, len(order))
		for _, k := range order {
			g := agg[k]
			// 聚合之后 reason 换成"由多少项资源构成"——原来那句"规格变化 a→b"
			// 是单个资源的事，硬凑到聚合行上会变成一句假话
			g["reason"] = fmt.Sprintf("%d 项资源", g["n"])
			agged = append(agged, g)
		}
		movers = agged
	default:
		httpx.Invalid(c, "dim", "resource|project|cluster|type")
		return
	}
	...
	// dim 回显：让"我选的维度到底生没生效"在响应里就能看出来 ——
	// 这个 bug 藏了这么久，正是因为响应里没有任何东西说得清它按什么聚合的
	dimOut := c.Query("dim")
	if dimOut == "" {
		dimOut = "resource"
	}
	c.JSON(http.StatusOK, gin.H{"month": month, "prev": prev, "dim": dimOut,
...
// StartCostSnapshotScheduler 每 6h 刷新当月成本快照。
//
// canRun 决定本轮要不要真的干活（多副本下只让 leader 跑）。
// 传 nil 表示不做约束 —— 单副本与本地开发是这个行为。
func StartCostSnapshotScheduler(st *store.Store, db *sql.DB, canRun func() bool) {
	h := &K8sCostHandler{Store: st, DB: db}
	for {
		...
		// 逐租户各自算各自的快照。绝不能用一个「看得见所有租户」的上下文
		// 跑一遍就完事 —— 那样聚合出来的成本会把别家的资源算进来。
		ctx := context.Background()
		rows, err := st.Platform("cost_snapshot").Query(ctx, store.ActiveTenantsQuery)
		if err != nil {
			logx.J("cost_snap", "list_tenants_err", map[string]any{"err": err.Error()})
			continue
		}
		...
		for _, tid := range tids {
			sc, err := st.Tenant(store.ForJob(ctx, tid, "cost_snapshot"))
			if err != nil {
				logx.J("cost_snap", "scope_err", map[string]any{"tenant_id": int64(tid), "err": err.Error()})
				continue
			}
			if n, err := h.takeSnapshot(sc, m); err != nil {
				logx.J("cost_snap", "err", map[string]any{"tenant_id": int64(tid), "month": m, "err": err.Error()})
			} else {
				logx.J("cost_snap", "ok", map[string]any{"tenant_id": int64(tid), "month": m, "rows": n})
			}
		}
	}
}
