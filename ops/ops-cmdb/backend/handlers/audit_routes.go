...
// auditStatusOf 把 HTTP 状态码翻成审计结果。判据只此一处。
//
// 4xx/5xx 也记 —— 失败的尝试同样是要审计的事实。
//
// ⚠️ **202 单独成一态，不能并进 success**。
//
//	域名同步、主机同步、批量续费都是「同步返回 202 + 后台 goroutine 真干活」，
//	真正的结果一两分钟后才落到「执行记录」里。
//	按 `< 400 就是 success` 判的话，审计会在**操作还没开始**时写下一条绿色的成功。
//
//	实测代价（OPSCMDB-009）：域名同步的 GoDaddy 凭据 401，执行记录里写着 fail，
//	审计日志上却是 success —— 同一件事两条互相矛盾的记录，
//	人信了显眼的那条，401 被绿色盖了近 20 分钟。
//	duration_ms 是铁证：全量同步要一两分钟，那条只有 11ms —— 那就是「已受理」本身。
//
//	accepted 的含义是「结果还不知道，去看执行记录」，既不谎报成功也不冤枉成失败。
func auditStatusOf(code int) string {
	switch {
...
var auditRoutes = map[string]auditRoute{
	...
	// ⚠️ 自己改自己的密码同样要留痕。原来漏了这条 —— 启动自检打了
	// 「未登记: PUT /api/me/password」的 WARN，但自检不阻断启动，
	// 于是这条 WARN 在日志里躺着没人管，而改密码这个动作**一条审计都没有**。
	"PUT /api/me/password":     {"user.change_own_password", "user", "", "", "", "none"},
	"POST /api/users/:id/kick": {"user.kick", "user", "", "id", "", "none"},
	"DELETE /api/users/:id":    {"user.delete", "user", "", "id", "", "none"},
	// ⚠️ 建账号和改角色原来都**没有任何审计**。
	// 这两个恰恰是最需要留痕的：谁给谁开了账号、谁把谁提成了管理员。
	// 启动自检当时是打了 WARN 的，但自检不阻断启动，那行 WARN 在日志里躺着没人看。
	"POST /api/users":         {"user.create", "user", "users", "", "INSERT", "local"},
	"PUT /api/users/:id/role": {"user.change_role", "user", "users", "id", "UPDATE", "local"},
	...
	// ⚠️ 下面四条原来一条都没登记。前两条尤其要命：
	//   激活授权 —— 谁在什么时候把这套系统换成了另一档，卖出去之后这是合规问题；
	//   改 SSO 配置 —— 改掉 issuer 就等于换了整个身份来源。
	// 都做成动作型（不留行快照、不可回滚）：
	//   license 的回滚等于降级，SSO 配置带密钥不该进快照，
	//   命名空间归属是批量结果、逐行回滚没有意义。
	// ⚠️ 动作型不等于不记 —— 谁、什么时候、改了什么，这三样一样不少。
	"POST /api/license":              {"license.activate", "license", "", "", "", "none"},
	"PUT /api/idp-config":            {"idp.update", "idp", "", "", "", "none"},
	"POST /api/idp-config/discover":  {"idp.discover", "idp", "", "", "", "none"},
	"POST /api/k8s/ns-projects/auto": {"ns_project.auto", "ns_project", "", "", "", "none"},
...
func AuditMiddleware(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		status := auditStatusOf(c.Writer.Status())
