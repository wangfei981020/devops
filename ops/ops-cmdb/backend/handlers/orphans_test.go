...
func TestParseBackendRef(t *testing.T) {
	cases := []struct {
		...
		{"user-client-backend-svc.app-user.svc.cluster.local", "other", "user-client-backend-svc", "app-user"},
		{"nacos.devops", "other", "nacos", "devops"},
		{"local-svc", "app-game", "local-svc", "app-game"}, // 无点号时归属调用方所在 ns
	}
	for _, c := range cases {
...
func TestIsSystemNamespace(t *testing.T) {
	...
	// 这些空了就是真该清理的
	for _, ns := range []string{"falco", "kyverno", "elkeid", "app-uat", "ls-uat"} {
		if isSystemNamespace(ns) {
