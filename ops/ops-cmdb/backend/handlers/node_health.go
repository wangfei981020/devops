...
func nodeHealthWatchCore(ctx context.Context, db *sql.DB, pool *k8ssource.Pool, cipher *crypto.Cipher, prog ProgressFn) (string, []TaskFailure, bool) {
	...
	for i, cl := range clusters {
		...
		// 磁盘趋势：只有接了 Prometheus 的集群能做（没接的会走 skip 分支）
...
// diskAlerts 用 predict_linear 外推节点磁盘何时写满。
// 比自己存历史采样点简单也准确得多，Prometheus 原生支持。
//
// 🔴 这个函数以前有**两个各自独立、都让它静默失效**的 bug，别再写回去：
//
//  1. clusterLabel 是**标签名**（如 `cluster`），不是完整选择器。
//     原来写 `sel = clusterLabel + "," + sel`，拼出 `{cluster,mountpoint="/"}` ——
//     这是 PromQL 语法错误，实测 HTTP 422。于是**所有配了 cluster_label 的集群**
//     （UAT / g32-prod / infra-01，即全部接 VM 的集群）满盘预测从来没产出过一条告警，
//     只有没配 cluster_label 的 DEV 跑得动。要用 clusterSelector() 拼。
//
//  2. mountpoint="/" 在 GKE COS 上量的是只读启动镜像，不是会满的那块盘。
//     口径见 nodefs.go。
func diskAlerts(db *sql.DB, cipher *crypto.Cipher, cid int, cname, provider, env string, now time.Time) ([]nodeAlert, error) {
	...
	cs := clusterSelector(db, clusterLabel, cid)
	lbl := nodeFsLabels(cs)
	// 24 小时内 avail 会跌破 0 = 预计满盘。只看可写的节点级文件系统。
	q := fmt.Sprintf(`predict_linear(node_filesystem_avail_bytes%s[6h], %d) < 0%s`,
		lbl, int(diskPredictWindow.Seconds()), nodeFsWritableSuffix(lbl))
	...
	// ⚠️ 走 nodeFsUsage 而不是自己再拼一遍查询 —— 这个函数就是被"各写一份"坑掉的。
	fs, _ := nodeFsUsage(base, token, cs)
	...
	for _, s := range samples {
		...
		v := fs[node]
		pct := v.Pct
		// 挂载点必须写出来：不写的话人无从判断我们量的是哪块盘，
		// 而"量错了盘"正是这条告警此前失效的原因之一。
		where := v.Mountpoint
		if where == "" {
			where = s.Metric["mountpoint"]
		}
		a := nodeAlert{
			ClusterID: cid, Cluster: cname, Provider: provider, Node: node,
			Kind: "disk_full", Level: "yellow", DiskPct: pct,
			Detail: fmt.Sprintf("%s 已用 %.1f%%，按当前写入速率预计 %s 内写满",
				where, pct, humanDur(diskPredictWindow)),
...
// larkWebhookForTask 取某个定时任务的飞书投递出口。
//
//	# ⚠️ 两级：任务绑的群优先，其次全局兜底
//
//	原来这里只查任务绑的 `lark_group_id`，**压根没读全局兜底**
//	（settings.feishu_webhook）。而基础配置页上写的是
//	  「全局兜底的通知出口。具体任务绑的群优先于它。」
//	界面承诺了兜底，代码没实现 —— 于是全局出口配得好好的，
//	到期提醒和升级提醒却双双报「未配置群，跳过」（OPSCMDB-031 P0-20）。
//
//	这类缺陷比"功能没做"更难发现：配置页看着是配好的，任务状态是"成功"
//	（跳过不算失败），只有点进结果详情才看到"跳过"两个字。
//	实际后果是 5 个域名的到期提醒一直没人收到。
//
//	返回的 group 用于日志和结果文案，走兜底时明确写成「全局兜底出口」——
//	不能让人以为消息发进了某个具体的群。
func larkWebhookForTask(db *sql.DB, taskKey string) (webhook, group string) {
	...
	if db.QueryRow(`SELECT lark_group_id FROM scheduled_tasks WHERE task_key=?`, taskKey).Scan(&gid) == nil && gid.Valid {
		if db.QueryRow(`SELECT name, webhook FROM lark_groups WHERE id=?`, gid.Int64).Scan(&group, &webhook) == nil && webhook != "" {
			return webhook, group
		}
		// 绑了群但取不出 webhook（群被删了、或 webhook 字段是空的）：
		// 这不是"没配"，是配了但坏了。落日志，然后照样走兜底 ——
		// 静默走兜底会让人一直以为消息进的是他绑的那个群
		logx.J("notify", "task_group_broken", map[string]any{
			"task": taskKey, "group_id": gid.Int64,
			"note": "任务绑的飞书群取不到 webhook，改用全局兜底出口",
		})
	}
	if fb := getSetting(db, "feishu_webhook"); fb != "" {
		return fb, "全局兜底出口"
	}
	return "", ""
}
...
func nodeHealthTaskNote(found, enabled bool) string {
	...
	if !enabled {
		return "任务当前是关闭的——这里的「无异常」不代表节点健康，只代表没在监控。" +
			"去「运行 → 定时任务」打开 node_health_watch，并给它配一个独立的告警群。"
	}
	return ""
}
