...
func NewRecordHandler(st *store.Store, db *sql.DB) *RecordHandler {
	return &RecordHandler{Store: st, DB: db}
}
...
func (h *RecordHandler) ListOriginRules(c *gin.Context) {
	...
	// 子查询里的计数同样要限定租户，否则"被几条解析用着"会把别家的算进来。
	rows, err := sc.Query(`SELECT r.id, r.cname, r.origin_ip,
			(SELECT COUNT(*) FROM domain_records dr WHERE dr.tenant_id = r.tenant_id AND LOWER(dr.cname)=LOWER(r.cname)) AS used,
			DATE_FORMAT(r.updated_at,'%Y-%m-%d %H:%i')
		FROM origin_ip_rules r WHERE r.tenant_id = ? ORDER BY used DESC, r.cname`)
	if err != nil {
...
func (h *RecordHandler) UpsertOriginRule(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&in); err != nil || strings.TrimSpace(in.Cname) == "" {
		httpx.Required(c, "origin_cname")
		return
	}
	if _, err := sc.Insert(`INSERT INTO origin_ip_rules (tenant_id, cname, origin_ip) VALUES (?, ?, ?)
...
func (h *RecordHandler) DeleteOriginRule(c *gin.Context) {
	...
	if _, err := sc.Exec(`DELETE FROM origin_ip_rules WHERE tenant_id = ? AND id=?`, c.Param("id")); err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"ok": true})
...
func (h *RecordHandler) ListAll(c *gin.Context) {
	...
	rows, err := sc.Query(`
		...
		WHERE r.tenant_id = ? AND c.type='domain' ` + cond + `
...
func (h *RecordHandler) CheckAllCerts(c *gin.Context) {
	...
	ciid := c.Param("ciid")
	var domain string
	if err := sc.QueryRow(`SELECT name FROM cis WHERE tenant_id = ? AND id=? AND type='domain'`, ciid).Scan(&domain); err != nil {
		httpx.NotFound(c, "domain")
		return
	}
	rows, err := sc.Query(`SELECT id, host FROM domain_records WHERE tenant_id = ? AND domain_ci_id=?`, ciid)
	if err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	for _, r := range recs {
		...
		go func(id int64, host string) {
			...
			if t, cmsg := tlsCertExpiry(fqdn); t != nil {
				_, _ = sc.Exec(`UPDATE domain_records SET cert_expiry_at=?, cert_check_at=NOW(), cert_check_msg=? WHERE tenant_id = ? AND id=?`, *t, truncate(cmsg, 250), id)
				atomic.AddInt32(&ok, 1)
			} else {
				_, _ = sc.Exec(`UPDATE domain_records SET cert_check_at=NOW(), cert_check_msg=? WHERE tenant_id = ? AND id=?`, truncate(cmsg, 250), id)
...
func (h *RecordHandler) List(c *gin.Context) {
	...
	rows, err := sc.Query(`
		...
		WHERE r.tenant_id = ? AND r.domain_ci_id=? ORDER BY r.stale, r.id`, c.Param("ciid"))
	if err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
		return
	}
...
func (h *RecordHandler) Create(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&in); err != nil || in.Host == "" {
		httpx.Required(c, "host")
		return
	}
	if in.RecordType == "" {
		in.RecordType = "A"
	}
	// 挂载的域名必须属于本租户，否则等于往别人的域名下加解析。
	var okDomain int
	if sc.QueryRow(`SELECT 1 FROM cis WHERE tenant_id = ? AND id=? AND type='domain'`, c.Param("ciid")).Scan(&okDomain) != nil {
		httpx.NotFound(c, "domain")
		return
	}
	res, err := sc.Insert(`INSERT INTO domain_records
		(tenant_id, domain_ci_id, host, record_type, cdn_id, cname, origin_ip, cert_expiry_at, project, env, module, operator)
		VALUES (?, ?, ?, ?, ?, ?, ?, NULLIF(?, ''), ?, ?, ?, ?)`,
...
func (h *RecordHandler) BulkUpdate(c *gin.Context) {
	...
	if len(sets) == 0 {
		httpx.FailKey(c, httpx.CodeBadRequest, "error.nothingToSet", nil, nil)
		return
	}
	...
	// ⚠️ 批量接口最容易漏：ids 是客户端给的，不加租户条件就等于
	//    "报上任意 id 即可批量改别人的解析记录"。
	q := "UPDATE domain_records SET " + strings.Join(sets, ", ") + " WHERE tenant_id = ? AND id IN (" + strings.Join(ph, ",") + ")"
	res, err := sc.Exec(q, args...)
	if err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
		return
	}
...
func (h *RecordHandler) BulkIgnore(c *gin.Context) {
	...
	q := "UPDATE domain_records SET ignored=?, ignore_reason=? WHERE tenant_id = ? AND id IN (" + strings.Join(ph, ",") + ")"
	res, err := sc.Exec(q, args...)
	if err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
		return
...
func (h *RecordHandler) Delete(c *gin.Context) {
	...
	if _, err := sc.Exec(`DELETE FROM domain_records WHERE tenant_id = ? AND id=?`, c.Param("id")); err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
		return
	}
	SetAuditTarget(c, c.Param("id"))
...
func (h *RecordHandler) CheckCert(c *gin.Context) {
	...
	if err := sc.QueryRow(`SELECT r.host, c.name FROM domain_records r JOIN cis c ON c.id=r.domain_ci_id WHERE r.tenant_id = ? AND r.id=?`, id).
