...
// ClusterName 这条告警**真正属于哪个集群**。
//
//	⚠️ 夜莺的顶层 `cluster` 字段名字骗人：它装的是**数据源名**
//	（实测生产上 279 条全是 "VictoriaMetrics"）。照它渲染「集群」列，
//	整页会显示成同一个集群，等于这一列没有信息量，还会让人以为
//	所有告警都来自一处。真集群在 tags.cluster 里（"ecs" /
//	"infra-k8s-cluster-01"）。
//
//	取不到就返回空 —— 不要退回数据源名冒充集群。
func (e AlertEvent) ClusterName() string {
	for _, k := range []string{"cluster", "kubernetes_cluster", "k8s_cluster"} {
		if v := e.TagsMap[k]; v != "" {
			return v
		}
	}
...
// sevSuffix 夜莺规则名尾部的级别标记：" - S1" / " - S2" / " - S3"。
var sevSuffix = regexp.MustCompile(`\s*-\s*S[123]\s*$`)

// ShortRuleName 去掉规则名尾部的级别后缀。
//
//	实测规则名形如 "HostHighDiskUsage - S3"，而级别已经有独立一列（info），
//	两处说同一件事，还挤占了列表最宝贵的横向空间。
//	只剥尾部的 S1/S2/S3，规则名里其它含 "S3" 的字样（比如 AWS S3 相关规则）
//	不在尾部，不会被误伤。
func (e AlertEvent) ShortRuleName() string {
	s := sevSuffix.ReplaceAllString(e.RuleName, "")
	if s == "" {
		return e.RuleName // 整个名字就是后缀这种畸形数据，宁可原样显示
	}
	return s
}
...
func (c *Client) CurrentAlerts(ctx context.Context, limit int) ([]AlertEvent, int64, error) {
	return c.CurrentAlertsSev(ctx, limit, 0)
}

// CurrentAlertsSev 同上，但可以把级别下推给夜莺。
//
//	# ⚠️ 为什么必须下推，不能只在本地过滤
//
//	夜莺是**先按 limit 截断再返回**的。原来的做法是拿回 limit 条再本地过滤级别，
//	于是 `severity=warning&limit=6` 会这样走：
//	  夜莺返回最新 6 条（恰好都是 info）→ 本地过滤 → 0 条
//	界面上就是「没有 warning 告警」，而真相是线上 187 条 warning 一条都没被取到。
//	**筛选把结果筛空了，看起来却像"这个级别很太平"**——这是本项目反复出现的那类
//	"出错了但不像出错"。
//
//	sev 传 0 = 不限。夜莺的 severity 参数值就是 1/2/3。
//	即使某些版本不认这个参数（会退化成返回全部），调用方的本地过滤仍然兜得住，
//	所以下推是安全的。
func (c *Client) CurrentAlertsSev(ctx context.Context, limit, sev int) ([]AlertEvent, int64, error) {
	if limit <= 0 || limit > 500 {
		limit = 200
	}
	path := fmt.Sprintf("/api/n9e/alert-cur-events/list?limit=%d&p=1", limit)
	if sev >= 1 && sev <= 3 {
