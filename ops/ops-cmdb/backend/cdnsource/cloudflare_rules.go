...
func (c *Client) ListPageRules(ctx context.Context, zoneID string) ([]Rule, error) {
	...
	if err != nil {
		logx.Line("cf-rules", fmt.Sprintf("ERROR zone=%s 取 Page Rules 失败: %v（若提示权限不足，token 需要 Zone·Page Rules·Read）", zoneID, err))
		return nil, err
	}
	var raw []struct {
		ID      string `json:"id"`
	...
	if err := json.Unmarshal(r.Result, &raw); err != nil {
		...
		logx.Line("cf-rules", fmt.Sprintf("ERROR zone=%s Page Rules 响应结构与预期不符: %v，原始片段: %s",
	...
	logx.Line("cf-rules", fmt.Sprintf("zone=%s Page Rules %d 条", zoneID, len(out)))
	return out, nil
}
...
func (c *Client) ListRulesets(ctx context.Context, zoneID string) ([]Rule, error) {
	...
	if err != nil {
		logx.Line("cf-rules", fmt.Sprintf("ERROR zone=%s 取 Rulesets 失败: %v（若提示权限不足，token 需要 Zone·Zone Settings·Read 或 Zone·Config·Read）", zoneID, err))
		return nil, err
	}
	var sets []struct {
		ID          string `json:"id"`
	...
	if err := json.Unmarshal(r.Result, &sets); err != nil {
		logx.Line("cf-rules", fmt.Sprintf("ERROR zone=%s Rulesets 响应结构与预期不符: %v，原始片段: %s",
			zoneID, err, trunc(string(r.Result), 300)))
	...
	logx.Line("cf-rules", fmt.Sprintf("zone=%s Rulesets 展开后 %d 条", zoneID, len(out)))
	return out, nil
}
...
func (c *Client) rulesetDetail(ctx context.Context, zoneID, rulesetID string) ([]Rule, error) {
	...
	if err := json.Unmarshal(r.Result, &detail); err != nil {
		logx.Line("cf-rules", fmt.Sprintf("ERROR zone=%s ruleset=%s 明细结构与预期不符: %v，原始片段: %s",
...
func (c *Client) ListCertificates(ctx context.Context, zoneID string) ([]Certificate, error) {
	...
	if err != nil {
		logx.Line("cf-rules", fmt.Sprintf("ERROR zone=%s 取证书包失败: %v（若提示权限不足，token 需要 Zone·SSL and Certificates·Read；"+
	...
	if err := json.Unmarshal(r.Result, &packs); err != nil {
		logx.Line("cf-rules", fmt.Sprintf("ERROR zone=%s 证书包响应结构与预期不符: %v，原始片段: %s",
			zoneID, err, trunc(string(r.Result), 300)))
	...
	logx.Line("cf-rules", fmt.Sprintf("zone=%s 证书包 %d 个", zoneID, len(out)))
	return out, nil
}
