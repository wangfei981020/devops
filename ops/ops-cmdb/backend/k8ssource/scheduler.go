...
// StartScheduler 周期全量同步所有启用集群。
//
// canRun 决定本轮要不要真的干活（多副本下只让 leader 跑）。
// 传 nil 表示不做约束 —— 单副本与本地开发是这个行为。
//
//	⚠️ 判断放在**循环内**而不是循环外：leader 是会变的
//	（滚动更新、租约易主）。放在外面的话，启动时不是 leader 的副本
//	会永远不干活，即使后来当选了。
func StartScheduler(db *sql.DB, pool *Pool, intervalSec int, canRun func() bool) {
	if intervalSec <= 0 {
		intervalSec = 120
	}
