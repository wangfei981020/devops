import zh from '@ops/i18n/locales/zh-CN/common.json'
...
/** 从语言包里按 `common:license.x` 取值，用于比对文案是否真的各不相同。 */
...
describe('licenseBanner', () => {
  ...
  it('七种状态的文案必须两两不同', () => {
    const texts = ALL.map((s) => {
      ...
      return b ? copy(b.key) : copy(`common:license.statusHint.${s}`)
    })
