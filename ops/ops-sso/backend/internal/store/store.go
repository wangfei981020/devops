...
// tenantArgIndex 算出租户参数该插在第几位。
//
// # 为什么不能一律放第一位
//
// 原先是无条件 prepend，隐含假设「tenant_id = ? 一定是语句里第一个 ?」。
// SELECT 基本都成立（WHERE 打头就是它），**UPDATE 不成立**：
//
//	UPDATE t SET a = ?, b = ? WHERE tenant_id = ? AND id = ?
//	              ↑1     ↑2              ↑3          ↑4
//
// 前置的话，租户值绑到了 a 上，后面每个参数整体错位一位 ——
// 而这**不会报语法错**，只会在某个类型对不上的位置炸出一句莫名其妙的话。
// 实测撞到的是：`Truncated incorrect DOUBLE value: '2026-08-10 22:48:10'`
// ——时间戳被绑到了 tenant_id 上。查这种错要从报错的字段一路倒推回参数顺序，
// 而报错里根本没提到租户。
//
// 所以按**占位符实际位置**插入：数一下 tenant_id = ? 之前有几个 ?，
// 那就是它的下标。tenant 在最前面时结果与旧行为完全一致。
//
// ⚠️ 数 ? 用的是字符扫描，SQL 里的字符串字面量若含 ? 会数偏。
// 本仓所有语句的字面量都不含 ?，且 checkFilter 已要求参数化写法；
// 真要写含 ? 的字面量，请改用 Raw。
func tenantArgIndex(query string) int {
	loc := tenantFilter.FindStringIndex(query)
	if loc == nil {
		return 0
	}
	return strings.Count(query[:loc[0]], "?")
}

// insertAt 把租户 ID 插进 args 的第 i 位。
func insertAt(id TenantID, args []any, i int) []any {
	if i <= 0 || i > len(args) {
		// i 超出范围只可能是数错了 ?。退回前置：那是旧行为，
		// 至少对绝大多数语句是对的，而不是把参数丢掉。
		return prepend(id, args)
	}
...
// scopedArgs 组装带租户的参数表。
func (s *Scoped) scopedArgs(query string, args []any) []any {
	return insertAt(s.tenant, args, tenantArgIndex(query))
}
...
func (s *Scoped) Query(query string, args ...any) (*sql.Rows, error) {
	...
	return s.raw.QueryContext(s.ctx, query, s.scopedArgs(query, args)...)
}
...
func (s *Scoped) QueryRow(query string, args ...any) *sql.Row {
	...
	return s.raw.QueryRowContext(s.ctx, query, s.scopedArgs(query, args)...)
}
...
func (s *Scoped) Exec(query string, args ...any) (sql.Result, error) {
	...
	return s.raw.ExecContext(s.ctx, query, s.scopedArgs(query, args)...)
}
...
func (s *Scoped) Insert(query string, args ...any) (sql.Result, error) {
	if !insertPattern.MatchString(query) {
		return nil, fmt.Errorf("%w: INSERT 的列清单里必须显式含 tenant_id: %s",
			ErrMissingTenantFilter, firstLine(query))
	}
	return s.raw.ExecContext(s.ctx, query, prepend(s.tenant, args)...)
}
...
// onDupKeyUpdate 匹配 `ON DUPLICATE KEY UPDATE`。
//
// 不剥掉它的话，`INSERT ... ON DUPLICATE KEY UPDATE secret_enc = ...` 里的
// `UPDATE secret_enc` 会被 tableRef 当成"操作了一张叫 secret_enc 的表"，
// 于是白名单把一条完全正常的语句判成越界 —— 一次真实的误报。
//
// 误报比漏报更糟：防线一旦开始喊狼来了，人就会绕过它。修的是检查器，不是绕过检查。
var onDupKeyUpdate = regexp.MustCompile(`(?is)\bon\s+duplicate\s+key\s+update\b`)

func checkPlatformTables(query string) error {
	// 只保留 ON DUPLICATE KEY UPDATE 之前的部分：后面是赋值列表，不含表名
	if loc := onDupKeyUpdate.FindStringIndex(query); loc != nil {
		query = query[:loc[0]]
	}
...
func prepend(id TenantID, args []any) []any {
	out := make([]any, 0, len(args)+1)
	out = append(out, int64(id))
