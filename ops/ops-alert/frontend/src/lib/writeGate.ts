 ...
 *
 * ⚠️ 与 CMDB 的同名文件相比这里少了 read-only 一档（授权降级）。
 * 等 licensekit 接进来要补上，不要把它和 no-permission 合并成一个"不能点"。
 */
export type WriteBlock = 'none' | 'pending' | 'no-permission'
...
export function useWriteGate(permCode: string): {
  ...
  if (!can(me.data, permCode)) {
    return {
      allowed: false,
      block: 'no-permission',
      reason: t('opsalert:perm.needCode', { code: permCode }),
    }
  }
