 ...
 *
 * 只有把两侧对起来才看得见。ops-cmdb 的域名模块就是这么漏的：
...
const ALLOW = new Map([
  ...
  ['POST /api/v1/auth/login', 'ops-alert 登录页直接用 fetch，不走统一的 api() 封装'],
  ['POST /api/v1/incidents/:p/ack', 'ops-alert 认领已按需求下线：接口与权限码保留（是"先不做"不是"不做"），界面上无入口'],
...
for (const prod of TARGETS) {
  ...
  for (const f of walk(beDir)) {
    ...
    // 路由组的前缀**必须从源码解析**，不能假设。
    //
    // ⚠️ 原来这里写死 `/api`，而四个产品里只有 ops-cmdb 是 /api，
    // 另外三个都是 /api/v1。于是 ops-video-manager 的 `api.POST("/tokens")`
  ...
  // ops-alert 的 lib/api.ts 导出 post/put/del，页面写的是
