 ...
 * # 为什么需要这个守卫
 *
 * OPSCMDB-031 的验证里，**八条问题是同一个根因**：前端接口类型里的字段名
 ...
 *	实测就撞到了——ops-sso 有两条真问题（见下），如果不分范围，
 *	ops-cmdb 的构建会一直红着，而红着的守卫最后只会被人加 `|| true` 绕过。
 *
...
OPSCMDB-031 里因此出过 8 条问题，最危险的一条把「申请 12000m」显示成「申请 0」，
