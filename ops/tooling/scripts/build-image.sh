...
#
# 用法：
#   tooling/scripts/build-image.sh ops-cmdb v12                    # 前端（默认）
#   tooling/scripts/build-image.sh ops-sso  v12 backend            # 后端
#   tooling/scripts/build-image.sh ops-cmdb v12                    # 前端（默认）
#   tooling/scripts/build-image.sh ops-sso  v12 backend            # 后端 --push        # 推 Harbor（单架构）
#   tooling/scripts/build-image.sh ops-cmdb v12                    # 前端（默认）
#   tooling/scripts/build-image.sh ops-sso  v12 backend            # 后端 --push-remote # 推 marks26（默认只 amd64）
#   ... --push-remote --multiarch                                   # 需要 arm64 时才加
#   ... --release                                                   # 发布：构建 amd64 → 扫这一份 → 通过才推 marks26
...
# 加了组件参数之后，`build-image.sh ops-cmdb v1 frontend --push` 里
...
case "$COMPONENT" in
  ...
  backend)
    ...
    #
    #   有 replace（如 ops-cmdb → ops-kit/licensekit）
    #     上下文必须是 ops/，否则那个目录不在里面，报
    #     「failed to compute cache key: "/ops-cmdb/backend": not found」
    #
    #   无 replace（如 ops-sso，暂未接 licensekit）
...
LOCAL_IMAGE="localhost:8070/opsplatform-dev/${PRODUCT}-${COMPONENT}:${VERSION}"
REMOTE_IMAGE="marks26/${PRODUCT}-${COMPONENT}:${VERSION}"
...
case "$MODE" in
  --push-remote)
    # 远端（marks26）是**送生产的通道**，生产 GKE 全是 linux/amd64，
    # 所以默认只推 amd64。
    #
    # 🔴 这里原来固定推 amd64+arm64，理由写的是"本地是 arm64"——但那条理由不成立：
    # 本地部署拉的是 localhost:8070 的 Harbor（见下面的 --push 分支，原生 arm64 构建），
    # **从来不从 marks26 拉**。于是那份 arm64 上传上去没有任何消费者。
    #
  ...
  --release)
    ...
    #	`--push-remote` 再构建一次 **amd64** 推 marks26。
    #	实测确认过：
    #	    本地(被扫的那份)   linux/arm64
    #	    marks26(上生产的)  linux/amd64
    ...
    if [[ $SCAN_RC -eq 1 ]]; then
      echo >&2
      echo "✗ Trivy 扫出未处置漏洞，已中止 —— **镜像没有推到 marks26**" >&2
      echo "  修依赖，或在 $IGNORE_FILE 里登记豁免并写明理由（ops/SECURITY-WAIVERS.md）" >&2
      exit 1
    elif [[ $SCAN_RC -ne 0 ]]; then
      echo >&2
      echo "✗ Trivy **没能完成扫描**（退出码 $SCAN_RC），已中止 —— **镜像没有推到 marks26**" >&2
