...
// 而 ops-alert 建出来之后没人补，于是它完全不受这个守卫约束，
 ...
 * 实测撞到两次（ops-alert 正在加页面，ops-cmdb 因此构建不了）。
 * 而挡不住的时候人会去加 `|| true`，那比收窄糟得多。
 *
 * 但默认**绝不能**变成只扫当前产品：`lib/products.mjs` 的注释里已经
 * 记过一次教训 —— 三个守卫各写一份硬编码清单，ops-alert 建出来后
 ...
 *
 * 语言包有两处合法位置，两种都在用：
 *   packages/i18n/locales/     共享包 —— ops-cmdb / ops-sso / ops-alert
 *   <产品>/frontend/src/locales 产品内 —— ops-video-manager
 *
