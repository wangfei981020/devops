...
for (const prod of products) {
  ...
  //
  // ⚠️ 三个产品有三种写法，全都合法：
  //   ops-video-manager  const resources = {  每行一个命名空间
  //   ops-cmdb           const resources = {  47 个全塞一行
  //   ops-alert          createI18n({ ... })  内联，压根没有 resources 变量
  // 所以判据只能锚在**语言标签**上 —— 那是 Resources 类型强制的形状，
  // 三种写法里都一样。
  //
  // ⚠️ 之前锚在 'const resources' 上，对 ops-alert 时 indexOf 返回 -1、
