...
func TestKiteWildcardExpandsToExactNS(t *testing.T) {
	...
	// 混合规则：一个通配 + 一个确切名 —— 正是用户的配法（g32-* + g66-uat）
	...
	if !hasExact {
		t.Error("确切名 argocd 被通配拖没了 —— 这就是 g66-uat 丢失的形态")
	}
...
func TestKiteExpandsFromRoleWhenNamespacesForbidden(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/api/auth/user":
			_, _ = w.Write([]byte(`{"user":{"username":"qa-read","roles":[
				{"name":"version-reader","clusters":["uat-k8s-cluster-01","g32-prod-cluster"],
				 "namespaces":["g32-uat","g32-pay","g66-uat","other-ns"],
	...
	// 用户的配法：一个通配 + 一个确切名
	rules := Rules{NS: NSRules{Include: []string{"g32-*", "g66-uat"}}}
	got, _ := k.resolveNamespaces(context.Background(), "uat-k8s-cluster-01", rules)

	want := map[string]bool{"g32-uat": true, "g32-pay": true, "g66-uat": true}
	if len(got) != len(want) {
		t.Fatalf("展开结果 %v，要 %v —— other-ns 不该进来，g66-uat 不该丢", got, want)
	}
	for _, n := range got {
		if !want[n] {
			t.Errorf("多展开了 %q", n)
	...
	for _, p := range asked {
		if strings.HasPrefix(p, "/api/v1/_clusters/uat-k8s-cluster-01/") {
...
func TestKiteRoleWildcardFallsBackToFullScan(t *testing.T) {
	...
	ns, expanded := k.resolveNamespaces(context.Background(),
		"c", Rules{NS: NSRules{Include: []string{"g32-*"}}})
	if ns != nil || expanded {
