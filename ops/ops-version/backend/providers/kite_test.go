...
func TestNSRules(t *testing.T) {
	cases := []struct {
		...
		{"空规则=全收", NSRules{}, "g32-wallet", true},
		{"前缀匹配", NSRules{Include: []string{"g32-*"}}, "g32-wallet", true},
		{"前缀不匹配", NSRules{Include: []string{"g32-*"}}, "xyz-core", false},
		{"精确匹配", NSRules{Include: []string{"g32-test"}}, "g32-test", true},
		{"精确不匹配前缀", NSRules{Include: []string{"g32-test"}}, "g32-test2", false},
		// 🔴 这条是重点：g32-* 会把 g32-uat 一起抓进来 → 同名冲突。exclude 是唯一的挡法
		{"exclude 优先于 include", NSRules{Include: []string{"g32-*"}, Exclude: []string{"g32-uat"}}, "g32-uat", false},
...
func TestBuildSnapshots(t *testing.T) {
	...
	rows := []rawWorkload{
		{"g32-base", "go-archive-server-backend-api", "deployments", "h/g32/go-archive-server-backend:20260813081348-14"},
		{"g32-base", "go-archive-server-backend-consumer", "deployments", "h/g32/go-archive-server-backend:20260813081348-14"},
		{"g32-base", "go-archive-server-backend-worker", "deployments", "h/g32/go-archive-server-backend:20260813081348-14"},
		{"g32-wallet", "wallet-client-backend", "deployments", "h/g32/wallet-client-backend:20260519082034-58ac8c3-114"},
	}
	...
	rows[2].Image = "h/g32/go-archive-server-backend:20260701000000-9"
	snaps = buildSnapshots(rows, nil)
	...
	rows = append(rows, rawWorkload{"g32-base", "nginx", "deployments", "docker.io/library/nginx:stable"})
