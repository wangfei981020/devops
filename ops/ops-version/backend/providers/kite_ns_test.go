...
func TestKitePerNamespaceMatchesFullScan(t *testing.T) {
	...
	nss := []string{"opsplatform", "ops-version", "argocd"}
	...
	wildcardNS := []string{"opsplatform", "ops-version", "argocd*"}
	...
	for k2, w := range wantSvc {
		...
		//    本地实测撞到的是 opsplatform/redis(7-alpine) 与
