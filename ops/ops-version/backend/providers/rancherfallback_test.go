...
func TestRancherFallsBackToPodsWhenDeploymentsForbidden(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.Contains(r.URL.Path, "/apis/apps/v1/"):
			...
			_, _ = w.Write([]byte(`{"message":"deployments.apps is forbidden: User \"u-x\" cannot list resource \"deployments\" in API group \"apps\" in the namespace \"g32-uat\"","code":403}`))
		case strings.HasSuffix(r.URL.Path, "/pods"):
			_, _ = w.Write([]byte(`{"items":[
				{"metadata":{"name":"wallet-backend-7d4f8b9c6d-x2k9p","namespace":"g32-uat"},
				 "spec":{"nodeName":"n1"},
				...
				{"metadata":{"name":"mysql-0","namespace":"g32-uat"},
				 "spec":{"nodeName":"n2"},
	...
	res, err := r.ListServices(context.Background(), "local",
		Rules{NS: NSRules{Include: []string{"g32-uat"}}}, false)
	if err != nil {
