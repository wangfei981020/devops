...
func TestRancherForbiddenHint(t *testing.T) {
	...
	cluster := rancherForbiddenHint(
		...
		`{"message":"deployments.apps is forbidden: User \"u-t2k9x\" cannot list resource \"deployments\" in API group \"apps\" at the cluster scope"}`)
	...
	single := rancherForbiddenHint(
		"/k8s/clusters/local/apis/apps/v1/namespaces/g32-uat/deployments", "")
	if !strings.Contains(single, "g32-uat") {
