...
func matrixVerdict(cells []matrixCell) string {
	...
	//
	//  ① 一开始「无法判定」排最前。三列里一列采集失败，
	//     整张表每一行都成了「无法判定」，而好几行在另外两列之间
	//     明明差着版本。→ 一个采不到的列不该污染整张表。
	//
	//  ② 改完之后「不一致」排在「缺失」前。真数据一跑：
	//     判为「不一致」的 26 行**全部**同时含缺失格子 ——
	//     这些服务在一半平台上压根不存在，而结论只说"版本不同"。
	//
	...
	//	两害相权，选不会骗人的那个。
	//
	// ⚠️ 「无法判定」仍排在两者之后：它说的是"不知道"，
	//    而上面两个是已经查实的事实。不知道不该盖掉已知。
	if missing {
		return concMissing
	}
	if hasDiff(tags) {
