...
export function ReconPage({ session }: { session: Session }) {
  ...
  async function refreshIfNeeded(cols: { orgId: number; projectId: number; env: string; orgName: string }[]) {
  ...
  const savePlan = useMutation({
    mutationFn: async (mode: 'create' | 'update') => {
      ...
      return api<{ id: number }>(
        mode === 'create' ? '/api/plans' : `/api/plans/${planId}`,
        {
  ...
  const filtering = Boolean(keyword.trim() || verdictPick)
  const filterDesc = [
  ...
  const columns = useMemo<ColumnDef<Row, unknown>[]>(() => {
    ...
    const failedCols = new Set(
      (result?.unhealthy_columns ?? []).map(respColKey),
    )
    cols.forEach((c, idx) => {
        ...
        {() => (
          <>
            ...
            <div className="flex flex-col gap-2 rounded-lg border border-border bg-card p-3 md:flex-row md:flex-wrap md:items-center">
              ...
              {can(session, 'view') && (
                <Button variant="primary" onClick={() => compare.mutate()} disabled={compare.isPending}>
            ...
            {outcomes.some((o) => o.state === 'failed') ? (
              <Banner tone="warn">
                {t('opsversion:recon.staleCols', {
                  cols: outcomes.filter((o) => o.state === 'failed')
                    .map((o) => o.label).join('、'),
                })}
              </Banner>
            ) : null}
            ...
            <div className="rounded-lg border border-border bg-card p-3">
              <div className="mb-2 text-[11px] text-muted-foreground">{t('opsversion:recon.columns')}</div>
                    ...
                    return (
                      ...
                      >
                        <span
                          ...
                          aria-hidden
                        />
                        <div className={`font-mono text-[17px] leading-tight font-semibold ${STAT[kind]}`}>
                          {n}
                        </div>
