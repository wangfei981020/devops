...
import {
  AsyncBoundary, Badge, Button, type ColumnDef, DataTable, EmptyState, TableSkeleton, fromQuery,
} from '@ops/ui'
...
interface AuditRow {
  /** 游标分页要用它 */
  id: number
  actor: string; action: string; target: string; detail: string
  result: string; error_msg: string; ip: string; created_at: string
}
...
export function AuditPage({ session }: { session: Session }) {
  ...
  const columns: ColumnDef<AuditRow, unknown>[] = [
    { id: 'time', header: t('opsversion:audit.time'), accessorFn: (r) => r.created_at,
      cell: ({ row }) => <span className="font-mono text-[11px] text-muted-foreground">
        {new Date(row.original.created_at).toLocaleString()}</span> },
    { id: 'actor', header: t('opsversion:audit.actor'), accessorFn: (r) => r.actor,
      cell: ({ row }) => <span className="text-xs text-foreground">{row.original.actor}</span> },
    { id: 'action', header: t('opsversion:audit.action'), accessorFn: (r) => r.action,
      cell: ({ row }) => <span className="font-mono text-xs text-foreground">{row.original.action}</span> },
    { id: 'target', header: t('opsversion:audit.target'), accessorFn: (r) => r.target,
      cell: ({ row }) => <span className="text-[11px] text-muted-foreground">{row.original.target || '—'}</span> },
    // 🔴 结果必须显示。只记「发起了操作」不记成败的话，
    //    一次 401 在审计里跟成功长得一模一样
    { id: 'result', header: t('opsversion:audit.result'), accessorFn: (r) => r.result,
      cell: ({ row }) => <Badge tone={row.original.result === 'success' ? 'ok' : 'bad'}>
        {row.original.result === 'success' ? t('opsversion:audit.success') : t('opsversion:audit.failed')}</Badge> },
    { id: 'detail', header: t('opsversion:audit.detail'), accessorFn: (r) => r.detail,
      cell: ({ row }) => (
        <div className="max-w-80 break-all">
          {/* detail 已在后端脱敏（password/api_key/token 一律 ***） */}
          <span className="font-mono text-[11px] text-muted-foreground">{row.original.detail}</span>
          {row.original.error_msg && (
            <div className="text-[11px] text-danger">{row.original.error_msg}</div>)}
        </div>) },
    { id: 'ip', header: 'IP', accessorFn: (r) => r.ip,
      cell: ({ row }) => <span className="font-mono text-[11px] text-muted-foreground">{row.original.ip}</span> },
  ]

  const state = fromQuery(q, (d) => (d.rows ?? []).length === 0, (e) => toLoadError(e, t))
  const page = q.data
  ...
  return (
    <div className="flex flex-col gap-3 p-5">
      ...
      <AsyncBoundary
        ...
        empty={<EmptyState title={t('opsversion:audit.empty')} reason={t('opsversion:audit.hint')} action={null} />}
