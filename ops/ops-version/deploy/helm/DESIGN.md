...
补上 leader election（参照 ops-cmdb 的 leases 表）之后才能开 autoscaling。
...
###   镜像仓库 harbor.slileisure.com/devops                               ###
###   入口 Istio · 只挂内网网关                                            ###
...
harbor.slileisure.com/devops/ops-version-backend:v0.45.0
...
若改成直接拉 marks26（imageRegistry: marks26），要另建：
kubectl -n <ns> create secret docker-registry marks26-pull \
--docker-server=docker.io --docker-username=marks26 --docker-password='<token>'
并把上面的 name 换成 marks26-pull
...
TLS 不用配：inner 网关 443 上已有 *.slileisure.com 与 *.sl-devops.com 通配证书
...
⚠️ 从 ops-cmdb 复制 chart 时踩过：那份 values 里写着 global.tag，
