...
镜像：`marks26/ops-version-backend:v0.2.0` / `marks26/ops-version-frontend:v0.2.0`
...
若要**直连 marks26**（镜像已推那儿），另建一个并改 registry：

```bash
kubectl -n devops create secret docker-registry marks26-pull \
  --docker-server=docker.io --docker-username=marks26 --docker-password='<token>'
```

装的时候加：`--set global.imageRegistry=marks26 --set 'global.imagePullSecrets[0].name=marks26-pull'`

否则先把两个镜像从 marks26 同步进生产 Harbor，values 不用改。
...
helm upgrade --install version ops/ops-version/deploy/helm \
  ...
  --set 'istio.hosts[0]=opsversion.sl-devops.com'
```
...
浏览器打开 `https://opsversion.sl-devops.com`，用 `SUPER_USER` / `SUPER_PASSWORD` 登录。
...
curl -X POST https://opsversion.sl-devops.com/api/orgs -b cookie.txt \
  -H 'Content-Type: application/json' -d '{
  "name":"我方","product":"g32平台","provider_type":"kite","auth_type":"password",
  "endpoint":"https://kite.sl-devops.com","username":"<账号>","password":"<密码>",
  "harbor_host":"harbor.slileisure.com","harbor_project":"g32","is_self":true,
  "envs":[
    {"env":"UAT","cluster_refs":["uat-k8s-cluster-01"],"ns_include":["g32-*"],
     "ns_exclude":["g32-uat"],"compare_enabled":true},
    {"env":"PROD","cluster_refs":["g32-prod-cluster"],"ns_include":["g32-*"],
...
```bash
curl https://opsversion.sl-devops.com/api/orgs/1/clusters?env=UAT -b cookie.txt
```
