...
func HandleListProjectEnvs(w http.ResponseWriter, r *http.Request) {
	...
	baseSQL := `SELECT id, name, display_name, env_type, git_repo, git_branch,
		...
		lark_webhook, lark_secret, lark_bot_id, gitlab_repo_id, auto_sync, IFNULL(ingress_gateway,''), IFNULL(harbor_project,''), IFNULL(domain_suffix,''), IFNULL(default_namespaces,''), IFNULL(zkv_secrets_path,''), created_at, updated_at
		FROM project_env`
	var rows *sql.Rows
	var err error
	if !enforce {
	...
	for rows.Next() {
		...
		_ = rows.Scan(&p.ID, &p.Name, &p.DisplayName, &p.EnvType, &p.GitRepo, &p.GitBranch,
			...
			&p.LarkWebhook, &p.LarkSecret, &p.LarkBotID, &p.GitlabRepoID, &p.AutoSync, &p.IngressGateway, &p.HarborProject, &p.DomainSuffix, &p.DefaultNamespaces, &p.ZkvSecretsPath, &p.CreatedAt, &p.UpdatedAt)
